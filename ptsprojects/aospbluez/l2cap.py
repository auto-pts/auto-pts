...
def test_cases(pts):
    ...
    test_cases = [
        TestCase("L2CAP", "TC_COS_CED_BV_01_C",
                 TestCmd("l2test -n -P 4113 %s" % pts.bd_addr(),
                         start_wid=49, stop_wid=14)),
        TestCase("L2CAP", "TC_COS_CED_BV_03_C",
                 TestCmd("l2test -y -N 1 -P 4113 %s" % pts.bd_addr(),
                         start_wid=49, stop_wid=14)),
        ...
        # the command is
        # btmgmt power off && btmgmt ssp off && btmgmt power on && \
        # l2test -r -P 4113 00:1B:DC:07:32:03 && btmgmt ssp on
        #
        # for some reason without power off I get
        # "Set Secure Simple Pairing for hci0 failed with status 0x0a (Busy)"
        # so problem with that command also ssp on won't run before l2test is
        # killed, which will kill whole command
        #
        # Hence, support for multiple commands is needed
        # run_test_case("L2CAP", "TC_COS_CED_BV_05_C", "btmgmt ssp off;
        # l2test -r -P 4113 %s; btmgmt ssp on" % (pts.bd_addr(),))
        ...
        TestCase("L2CAP", "TC_COS_CED_BV_11_C",
                 TestCmd("l2test -u -P 4113 %s" % pts.bd_addr(),
                         start_wid=49)),
        ...
        TestCase("L2CAP", "TC_COS_CFC_BV_01_C",
                 [TestFunc(btmgmt.advertising_off),
                  TestCmd("l2test -y -N 1 -b 40 -V le_public -P 37 %s" %
                          pts.bd_addr(), stop_wid=22)]),
        TestCase("L2CAP", "TC_COS_CFC_BV_02_C",
                 TestCmd("l2test -y -N 1 -b 1 -V le_public -P 37 %s" %
                         pts.bd_addr(), stop_wid=22)),
        TestCase("L2CAP", "TC_COS_CFC_BV_03_C",
                 TestCmd("l2test -u -V le_public -P 37 %s" %
                         pts.bd_addr(), stop_wid=22)),
        ...
        # TestCase("L2CAP", "TC_COS_CFC_BV_05_C",
        # "l2test -u -V le_public -P 37 %s" % pts.bd_addr(), stop_wid = 22),

        TestCase("L2CAP", "TC_CLS_UCD_BV_01_C"),
        TestCase("L2CAP", "TC_CLS_UCD_BV_02_C",
                 TestCmd("l2test -s -G -N 1 -P 4113 %s" % pts.bd_addr(),
                         start_wid=49)),
        TestCase("L2CAP", "TC_CLS_UCD_BV_03_C",
                 TestCmd("l2test -s -E -G -N 1 -P 4113 %s" % pts.bd_addr(),
                         start_wid=49)),
        ...
        TestCase("L2CAP", "TC_CMC_BV_12_C",
                 TestCmd("l2test -z -X ertm %s" % pts.bd_addr(),
                         start_wid=49)),
        TestCase("L2CAP", "TC_CMC_BV_13_C",
                 TestCmd("l2test -z -X streaming %s" % pts.bd_addr(),
                         start_wid=49)),
        ...
        TestCase("L2CAP", "TC_ERM_BV_07_C",
                 TestCmd("l2test -r -H 1000 -K 10000 -X ertm -P 4113",
                         start_wid=15),
                 no_wid=19),
        ...
        #          TestCmd("l2test -z -P 4113 %s" % pts.bd_addr(),
        #                  start_wid = 49))
        ...
        TestCase("L2CAP", "TC_LE_CFC_BV_03_C",
                 [TestFunc(btmgmt.advertising_on),
                  TestFunc(pts.update_pixit_param, "L2CAP",
                           "TSPX_iut_role_initiator", "FALSE"),
                  TestCmd("l2test -x -N 1 -V le_public %s" % pts.bd_addr(),
                          stop_wid=22),
                  TestFuncCleanUp(btmgmt.advertising_off),
                  TestFuncCleanUp(pts.update_pixit_param, "L2CAP",
