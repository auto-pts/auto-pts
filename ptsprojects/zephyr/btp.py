...
# Address
le_address = namedtuple('le_address', 'addr_type addr')
PTS_BD_ADDR = le_address(addr_type=0, addr='000000000000')
...
def gap_conn(bd_addr=None, bd_addr_type=None):
    ...
    # If address provided, remove colons from address.
    # Otherwise, use stored address.
    if bd_addr:
        bd_addr = "".join(bd_addr.split(':'))
    else:
        bd_addr = get_pts_addr()
        bd_addr_type = get_pts_addr_type()
...
def gap_disconnected_ev(bd_addr=None, bd_addr_type=None):
    ...
    # If address provided, remove colons from address and convert to lower case.
    # Otherwise, use stored address.
    if bd_addr:
