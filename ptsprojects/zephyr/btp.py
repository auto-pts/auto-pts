...
def l2cap_conn_rsp():
    ...
    global L2CAP_CONN
    L2CAP_CONN.append(chan_id)

    logging.debug("new L2CAP channel: id %r", chan_id)


def l2cap_diconn(conn_id):
    logging.debug("%s %r", l2cap_diconn.__name__, conn_id)

    zephyrctl = iutctl.get_zephyr()

    global L2CAP_CONN
    conn_id = L2CAP_CONN[L2CAP_CONN.index(conn_id)]

    data_ba = bytearray(chr(conn_id))
