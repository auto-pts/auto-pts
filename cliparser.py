...
class CliParser(argparse.ArgumentParser):
    def __init__(self, iut_modes=None, board_names=None, add_help=True, *args, **kwargs):
        ...
        self.add_argument("--no_retry_on_regression", type=bool,
                          help="When skip_retry is used, failed test cases are handled as follows: if test failure is "
                               "not a regression, test case will not be retried (i.e. retry is ignored). If the failure"
                               " is regression, test case  will be retried for retry number of times. If you set retry"
                               " to zero, no failed test cases will be retried.")
