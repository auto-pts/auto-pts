...
from autopts.pybtp.types import Perm, Prop, WIDParams
...
def gatts_wid_hdl(wid, description, test_case_name):
    ...
    if stack.is_svc_supported('GATTS') and 'GATT/SR' in test_case_name:
        return generic_wid_hdl(wid,
                               description,
                               test_case_name,
                               [__name__, 'autopts.ptsprojects.mynewt.gatts_wid',
                                'autopts.wid.gatts'])
    return generic_wid_hdl(wid,
                           description,
                           test_case_name,
...
def hdl_wid_92(params: WIDParams):
    ...
    btp.gatt_sr_set_val(bd_addr_type, bd_addr, [handle], [value])
...
def hdl_wid_98(params: WIDParams):
    ...
    values = [hexlify(value[::-1])]

    btp.gatt_sr_set_val(bd_addr_type, bd_addr, [handle], values)
...
def hdl_wid_111(_: WIDParams):
    ...
    for chrc in chrcs:
        ...
        prop, handle, chrc_uuid = struct.unpack(f"<BH{uuid_len}s", val)
        chrc_value_attr = btp.gatt_sr_get_attr_val(start_handle=handle,
                                                   end_handle=handle)
        if not chrc_value_attr:
            continue
