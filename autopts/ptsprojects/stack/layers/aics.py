...
class AICS:
    def __init__(self):
        self.event_queues = {
            defs.BTP_EV_AICS_STATE: [],
            defs.BTP_EV_AICS_GAIN_SETTING_PROP: [],
            defs.BTP_EV_AICS_INPUT_TYPE: [],
            defs.BTP_EV_AICS_STATUS: [],
            defs.BTP_EV_AICS_DESCRIPTION: [],
            defs.BTP_EV_AICS_PROCEDURE: [],
        }
    ...
    def wait_aics_state_ev(self, addr_type, addr, timeout, remove=True):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_AICS_STATE],
    ...
    def wait_aics_gain_setting_prop_ev(self, addr_type, addr, timeout, remove=True):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_AICS_GAIN_SETTING_PROP],
            lambda _addr_type, _addr, *_:
            (addr_type, addr) == (_addr_type, _addr),
            timeout, remove)
    ...
    def wait_aics_input_type_ev(self, addr_type, addr, timeout, remove=True):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_AICS_INPUT_TYPE],
            lambda _addr_type, _addr, *_:
            (addr_type, addr) == (_addr_type, _addr),
            timeout, remove)
    ...
    def wait_aics_status_ev(self, addr_type, addr, timeout, remove=True):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_AICS_STATUS],
            lambda _addr_type, _addr, *_:
            (addr_type, addr) == (_addr_type, _addr),
            timeout, remove)
    ...
    def wait_aics_description_ev(self, addr_type, addr, timeout, remove=True):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_AICS_DESCRIPTION],
            lambda _addr_type, _addr, *_:
            (addr_type, addr) == (_addr_type, _addr),
            timeout, remove)
    ...
    def wait_aics_procedure_ev(self, addr_type, addr, timeout, remove=True):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_AICS_PROCEDURE],
            lambda _addr_type, _addr, *_:
            (addr_type, addr) == (_addr_type, _addr),
            timeout, remove)
