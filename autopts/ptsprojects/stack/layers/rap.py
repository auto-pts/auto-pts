...
class RAP:
    def __init__(self):
        self.event_queues = {
            defs.BTP_RAP_EV_RANGING_COMPLETE: [],
        }

    def event_received(self, event_type, event_data):
        self.event_queues[event_type].append(event_data)

    def wait_ranging_complete_ev(self, addr_type, addr, timeout, remove=True):
        return wait_event_with_condition(
            self.event_queues[defs.BTP_RAP_EV_RANGING_COMPLETE],
