...
class ASCS:
    def __init__(self):
        self.event_queues = {
            defs.BTP_EV_ASCS_OPERATION_COMPLETED: [],
            defs.BTP_EV_ASCS_CHARACTERISTIC_SUBSCRIBED: [],
            defs.BTP_EV_ASCS_ASE_STATE_CHANGED: [],
        }
    ...
    def wait_ascs_operation_complete_ev(self, addr_type, addr, ase_id, timeout, remove=True):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_ASCS_OPERATION_COMPLETED],
    ...
    def wait_ascs_characteristic_subscribed_ev(self, addr_type, addr, timeout, remove=True):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_ASCS_CHARACTERISTIC_SUBSCRIBED],
    ...
    def wait_ascs_ase_state_changed_ev(self, addr_type, addr, ase_id, state, timeout, remove=True):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_ASCS_ASE_STATE_CHANGED],
