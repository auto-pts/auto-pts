...
from autopts.pybtp.types import BIPAppParamTag, BIPRemoteDisplay, OBEXHdr, OBEXRspCode
...
def _parse_xml_attr(xml_str, attr):
    if not xml_str:
        return None
    # The lookbehind anchors the attribute name so that a longer name *ending*
    # in attr is not matched: without it, _parse_xml_attr(x, "name") returns
    # the value of friendly-name="..." (BIP DTD 4.6.8 image-properties carries
    # both). A plain \b is not enough here, because there is already a word
    # boundary between the '-' and the 'n' of friendly-name; the class has to
    # exclude every character that may appear in an XML attribute name.
    m = re.search(rf'(?<![-\w.:]){re.escape(attr)}\s*=\s*"([^"]*)"', xml_str)
...
def bip_build_tlv(tlv_dict: dict, tag_sizes: dict) -> bytearray:
    """Build TLV-encoded bytearray from a tag->value dict.

    BIP owns its own copy of the OBEX TLV/header codecs: this used to be
    types.py:obex_build_tlv, a "generic" helper that BIP was in practice the
    only user of. Keeping it here means changing BIP's wire format cannot
    surprise another profile, and reading BIP's wire format does not mean
    reading a helper written for everyone. The body is unchanged from the
    version in types.py.
    ...
    for tag, value in tlv_dict.items():
        ...
        if size in _PACK_FMT:
            ...
            if isinstance(value, str):
                data = (bytes.fromhex(value) if len(value) == 32
                        else value.encode('utf-8')[:16].ljust(16, b'\x00'))
...
def _db_locked(method):
    """Run *method* with the image database's own lock held.

    Everything in BIPImageDatabase is reachable from two threads: the server
    auto-responses run on the BIP event-handler thread, while the WID handlers
    on the main thread send PUTs and read back what the auto-responses stored
    (ds.images, last_put_image, last_image_list, and the PUT reassembly state).
    The only synchronisation that existed was the happens-before that
    wait_for_operation_complete() incidentally creates, and several WID
    handlers do not wait at all - hdl_wid_36/37/39/40 send and return
    immediately - so those paths had no synchronisation point whatsoever.

    The lock is an RLock and is exposed as BIPImageDatabase.lock, so a caller
    that needs several of these calls to be one atomic step (read
    last_put_image, mutate the record, add it to ds) can hold it across them
    without deadlocking against the per-method locking here.
    """
...
def _request_body(headers):
    """Return every Body / End-Body payload of a request, in wire order.

    OBEX lets one packet carry several Body headers; the object they carry is
    their payloads concatenated. The handlers used to read
    `headers.get(BODY) or headers.get(END_BODY)`, which takes at most one of the
    two - so a packet carrying both silently lost the End-Body bytes, and the
    stored image ended up shorter than what arrived, with native_size matching
    the truncated data rather than the real object.

    headers is in wire order (bip_parse_headers fills it as it parses, and a
    dict preserves insertion order), so iterating it keeps the chunks in the
    order they were received. The client side has always reassembled this way -
    see _bip_extract_body_xml in pybtp/btp/bip.py, whose docstring spells out
    the "several Body headers followed by one End-Body" shape.

    Returns b'' when the packet carries no body at all, which the callers test
    for the same way they tested the old None.
    """
    return b''.join(value for header_id, value in headers.items()
                    if header_id in (OBEXHdr.BODY, OBEXHdr.END_BODY))


class BIPImageDatabase:
    """The image store the tester answers BIP server requests from.

    Every operation handler is named <operation>_rsp and takes the same two
    arguments, (final, headers):

        final    the OBEX final-bit of the request packet being answered
        headers  the parsed {OBEXHdr.X: value} dict of that packet

    Anything else a handler needs comes out of headers - in particular the
    Application Parameters are read as headers.get(OBEXHdr.APP_PARAM, {}) by
    the handlers that want them, not extracted by the caller and passed in.
    The signatures used to come in five shapes ((headers), (headers,
    app_params), (final, headers), (secondary_active)), which meant every one
    of the sixteen otherwise-identical event handlers in btp/bip.py had to be
    written against its own convention.

    Not every handler needs both arguments - GetCapabilities depends on neither
    and DeleteImage ignores final - but they are accepted uniformly so that the
    dispatch side has nothing to special-case.

    get_status_rsp is the one deliberate exception: what it answers from is not
    an OBEX header but live OBEX session state, derived for it by
    BIP.build_get_status_rsp.
    """

    def __init__(self):
        # Guards every mutable attribute of this object; see _db_locked().
        self.lock = RLock()
        ...
        # Reassembly state for the printer-control object of a StartPrint PUT,
        # and the IMG SRC file names extracted from it once complete. These were
        # the three attributes that really were created on first use, which is
        # why every reader had to reach for getattr(); declaring them here lets
        # all of them - and the five above, which were already initialised - be
        # read directly.
        self._print_body = b''
        self._print_body_in_progress = False
        self.print_img_srcs = None
    ...
    @_db_locked
    def get_caps_rsp(self, final, headers):
    ...
    @staticmethod
    def _parse_pixel_size(size):
        """Parse a "W*H" pixel size into (width, height), or None."""
        m = re.match(r'^\s*(\d+)\s*\*\s*(\d+)\s*$', size or '')
        if not m:
            return None
        return int(m.group(1)), int(m.group(2))

    @classmethod
    def _parse_pixel_spec(cls, spec):
        """Split a pixel spec into bounds, or None when it is not a valid spec.

        Spec Table 4.14 defines three forms, all of which appear in real
        capability objects and in PTS filtering parameters:

          "W*H"            a fixed size
          "W1*H1-W2*H2"    a range, modified aspect ratio allowed
          "W1**-W2*H2"     a range with a fixed aspect ratio: widths run from
                           W1 to W2 and the height of each intermediate width
                           is H=(W*H2)/W2

        Returns (lo, hi, fixed_aspect) with lo/hi as (width, height) pairs -
        lo[1] is None for the fixed-aspect form - or None if *spec* is none of
        the three. The third form used to fall through to None here, which made
        it match nothing at all; it is the reason this is a separate method the
        filter path can ask before it decides to filter.
        """
        if spec is None:
            return None

        if "-" not in spec:
            exact = cls._parse_pixel_size(spec)
            return None if exact is None else (exact, exact, False)

        lo_s, hi_s = spec.split("-", 1)
        hi = cls._parse_pixel_size(hi_s)
        if hi is None:
            return None

        lo = cls._parse_pixel_size(lo_s)
        if lo is not None:
            return lo, hi, False

        # "W1**": the width alone, the height follows the upper bound's ratio.
        m = re.match(r'^\s*(\d+)\s*\*\s*\*\s*$', lo_s)
        if m is None or hi[0] == 0:
            return None
        return (int(m.group(1)), None), hi, True

    @classmethod
    def _pixel_in_range(cls, pixel, spec):
        """Return True if pixel string matches a capabilities pixel spec.

        The spec may be any of the three forms of spec Table 4.14; see
        _parse_pixel_spec(). An unparsable spec (or an unparsable *pixel*)
        returns False - callers that must not drop data on a malformed spec
        should ask _parse_pixel_spec() first.
        """
        req = cls._parse_pixel_size(pixel)
        bounds = cls._parse_pixel_spec(spec)
        if req is None or bounds is None:
            return False

        lo, hi, fixed_aspect = bounds
        width, height = req

        if not lo[0] <= width <= hi[0]:
            return False

        if fixed_aspect:
            # H=(W*H2)/W2 per Table 4.14. Integer arithmetic, so allow the one
            # pixel the truncation can lose.
            return abs(height - (width * hi[1]) // hi[0]) <= 1

        return lo[1] <= height <= hi[1]
    ...
    @_db_locked
    def get_image_list_rsp(self, final, headers):
        ...
        f_pixel = _parse_xml_attr(img_desc_xml, "pixel")

        # The pixel filter is advertised as supported in get_caps_rsp
        # (filtering_parameters["pixel"] = "1"), and the BIP test spec requires
        # a filter advertised as supported to actually be applied: "the IUT
        # returns a GetImagesList response with a filtered images-listing object
        # and an image handles descriptor identical to the one received". It used
        # to be parsed nowhere and silently ignored, so the full list came back.
        #
        # A spec string that is none of the three forms of Table 4.14 is treated
        # as "filter not applied" rather than "matches nothing" - dropping every
        # image because of a malformed filter is the worse failure of the two,
        # and Table 4.13 lets a server ignore filtering parameters it does not
        # support.
        if f_pixel and self._parse_pixel_spec(f_pixel) is None:
            logging.warning('GetImagesList: ignoring unparsable pixel filter '
                            '%r', f_pixel)
            f_pixel = None
        ...
        for h in handles:
            ...
            if f_encoding and \
                ...
                continue
            if f_pixel and not self._pixel_in_range(img.native_pixel, f_pixel):
                continue
            filtered.append(h)
    ...
    @_db_locked
    def get_image_properties_rsp(self, final, headers):
    ...
    # === GetImage (4.5.8) ===
    @_db_locked
    def get_image_rsp(self, final, headers):
        img_handle = self._extract_handle(headers)
        img = self.ds.images.get(img_handle)
    ...
    @_db_locked
    def get_linked_thumbnail_rsp(self, final, headers):
        img_handle = self._extract_handle(headers)
        img = self.ds.images.get(img_handle)
    ...
    @_db_locked
    def get_linked_attachment_rsp(self, final, headers):
    ...
    @_db_locked
    def get_partial_image_rsp(self, final, headers):
        app_params = headers.get(OBEXHdr.APP_PARAM, {})
        ...
        total_size = len(data)
        # Distinguish "header absent" from "header present with value 0".
        # Per BIP v1.2.1 section 4.5.13 PartialFileLength spans 0x00000000 to
        # 0xFFFFFFFF, and only 0xFFFFFFFF means "the entire image"; `or` turned
        # an explicit request for 0 bytes into a request for the whole file.
        start = _parse_app_param_value(
            app_params, BIPAppParamTag.PARTIAL_FILE_START_OFFSET, '>I')
        if start is None:
            start = 0
        length = _parse_app_param_value(
            app_params, BIPAppParamTag.PARTIAL_FILE_LENGTH, '>I')
        if length is None:
            length = 0xFFFFFFFF
    ...
    @_db_locked
    def get_monitoring_image_rsp(self, final, headers):
    ...
    def put_image_rsp(self, final, headers):
        ...
        # 2. Start a new image record whenever no PUT is in progress.
        #
        #    This must NOT be gated on "not final": a single-packet PutImage
        #    arrives with final=True and no ongoing PUT, so gating on final made
        #    it skip record creation, then drop the body in step 3 (the guard
        #    there requires _current_put_handle) and finally return a handle
        #    straight from next_handle() in step 4 - a handle that is not in
        #    ds.images. Because next_handle() is pure (max(int(h)) + 1), the
        #    next real PutImage would then hand out that very same handle, so
        #    two images ended up sharing one handle and a GetImageProperties on
        #    the reported handle answered NOT_FOUND.
        if self._current_put_handle is None:
            handle = self.ds.next_handle()
            now = datetime.datetime.now(
                timezone.utc).strftime("%Y%m%dT%H%M%SZ")
        ...
        # 4. Always return the same Img-Handle for this image. Step 2
        #    guarantees _current_put_handle is set by the time we get here.
        img_handle = _handle_to_utf16(self._current_put_handle)
    ...
    def put_linked_thumbnail_rsp(self, final, headers):
        ...
        target = self._current_thm_handle
    ...
    def put_linked_attachment_rsp(self, final, headers):
        ...
        target = self._current_att_handle
        ...
        if target and target in self.ds.images:
            img = self.ds.images[target]
            meta = self._current_att_meta or {}
            att = None
    ...
    def start_print_rsp(self, final, headers):
        ...
        body = _request_body(headers)
        if body:
            if not self._print_body_in_progress:
                self._print_body = b''
...
class OBEXSession:
    ...
    # Largest OBEX packet the tester can physically build, whatever the peer
    # advertises. Its BIP tx pool buffers are sized
    # MAX(BT_RFCOMM_BUF_SIZE, BT_L2CAP_BUF_SIZE)(CONFIG_BT_GOEP_*_MTU)
    # (tests/bluetooth/tester/src/btp_bip.c), and CONFIG_BT_GOEP_*_MTU follows
    # CONFIG_BT_BUF_ACL_RX_SIZE, which prj_br.conf sets to 300.
    #
    # This is a genuinely separate limit from peer_mopl, not a restatement of it.
    # The tester does clamp the peer's advertised MOPL before reporting it, but
    # it clamps to obex->tx.mtu (obex.c), and for GOEP v2 that is the *peer's*
    # L2CAP MTU (goep.c: obex.tx.mtu = chan.tx.mtu) - which says nothing about
    # the tester's own buffers. A PTS MOPL of 1790 therefore arrives unclamped,
    # and a response budgeted from it is rejected outright by
    # alloc_buf_with_data_bip() ("payload of N does not fit in ... tailroom"):
    # the BTP command fails, it is not fragmented.
    IUT_MAX_MOPL = 300

    # Budget used before any CONNECT has recorded a MOPL. This is the value the
    # whole send path used to hard-code, so an unknown MOPL behaves exactly as
    # the code did before the MOPL was threaded through.
    FALLBACK_MOPL = 150
    ...
    def __init__(self, role=BIPObexRole.PRIMARY, connection=None):
        ...
        self.conn_id = None
        self.conn_info = {}
        # Maximum OBEX packet length the *peer* is willing to receive, recorded
        # from the CONNECT / CONNECT response event. Only half of the transmit
        # budget: see IUT_MAX_MOPL for the other half, and tx_mopl() for the
        # value callers should actually use. None until a CONNECT is seen.
        self.peer_mopl = None
        self.data_rx = {}
        self.data_tx = {}
        self.srm_flags = 0
    ...
    def tx_mopl(self):
        """Largest OBEX packet this session may transmit, in bytes.

        The smaller of what the peer accepts (peer_mopl, FALLBACK_MOPL before a
        CONNECT) and what the tester can build (IUT_MAX_MOPL). Both limits are
        real and neither implies the other.
        """
        return min(self.peer_mopl or self.FALLBACK_MOPL, self.IUT_MAX_MOPL)
...
class BIP:
    def __init__(self):
        # This import must stay inside the function, and it is what keeps the
        # rest of the BIP code free of function-local imports.
        #
        # pybtp/btp/bip.py depends on this module at module level - it needs
        # OBEXSession, the data records and the pure helpers defined above - and
        # that is the direction the dependency should run: the BTP protocol
        # module builds on the stack layer's data model. The one thing this
        # module needs from over there is BIPEventHandler, the BTP event-parsing
        # worker. Spelling that as a module-level import would close the loop
        # (this module -> btp.bip -> ptsprojects.stack -> layers -> this module)
        # and whichever of the two was imported first would fail.
        #
        # Importing it here instead means the loop is only ever traversed at
        # call time, when both modules are fully initialised. BIP() is built by
        # Stack.bip_init() during a test case's preconditions, long after import.
        #
        # Removing this last one would mean moving BIPEventHandler into this
        # layer and having btp/bip.py register BIP_EV_HANDLERS into it - a real
        # change to how events are dispatched, not an import tidy-up.
    ...
    def cleanup(self):
        """Release the resources this layer owns before it is replaced.

        Only the event-handler worker thread needs it: everything else is plain
        per-instance state that goes away with the object. Stack.bip_init() calls
        this on the outgoing layer, so the thread does not outlive the BIP object
    ...
    def has_pending_second_connect(self, address):
        ...
        The secondary server CONNECT request event is enqueued on the SECONDARY
        ...
        if conn is None:
            return False
        session = conn.get_session(BIPObexRole.SECONDARY)
        if session is None:
            return False
    ...
    def rx(self, address, ev, data, role=BIPObexRole.PRIMARY):
        conn = self.get_bip_connection(address)
        if conn is None:
            # Dropping the event here is the only thing left to do, but it is
            # never harmless: whichever wait_for_operation_complete() the test
            # case is sitting in will now time out, several WID handlers later.
            # Say so at the point of loss so the log shows the cause and not
            # only the symptom.
            logging.error('BIP.rx: no connection for %s, dropping event 0x%02x',
                          address, ev)
            return
        session = conn.get_session(role)
        if session is None:
            logging.error('BIP.rx: no %r session for %s, dropping event 0x%02x',
                          role, address, ev)
            return
        session.rx(ev, data)
