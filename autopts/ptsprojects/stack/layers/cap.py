...
class CAP:
    def __init__(self):
        self.event_queues = {
            defs.BTP_EV_CAP_DISCOVERY_COMPLETED: [],
            defs.BTP_EV_CAP_UNICAST_START_COMPLETED: [],
            defs.BTP_EV_CAP_UNICAST_STOP_COMPLETED: [],
        }
    ...
    def wait_discovery_completed_ev(self, addr_type, addr, timeout, remove=True):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_CAP_DISCOVERY_COMPLETED],
    ...
    def wait_unicast_start_completed_ev(self, cig_id, timeout, remove=True):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_CAP_UNICAST_START_COMPLETED],
            lambda _cig_id: cig_id == _cig_id,
            timeout, remove)

    def wait_unicast_stop_completed_ev(self, cig_id, timeout, remove=True):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_CAP_UNICAST_STOP_COMPLETED],
