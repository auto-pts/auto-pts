...
class HFP_HF:
    ...
    def wait_for_call_count_gte(self, addr, gte_count, timeout=10):
        """Wait for call count to be greater than or equal to gte_count."""
        wait_for_event(timeout, lambda: self.get_call_count(addr) >= gte_count)
        return self.get_call_count(addr) >= gte_count

    def wait_for_call_count_not_equal(self, addr, unexpected_count, timeout=10):
        """Wait for call count to be different from unexpected_count."""
        wait_for_event(timeout, lambda: self.get_call_count(addr) != unexpected_count)
        return self.get_call_count(addr) != unexpected_count
