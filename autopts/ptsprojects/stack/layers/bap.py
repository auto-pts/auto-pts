...
class BAP:
    ...
    def __init__(self):
        ...
        self.broadcast_code = ''
        self.event_queues = {
            defs.BTP_EV_BAP_DISCOVERY_COMPLETED: [],
            defs.BTP_EV_BAP_CODEC_CAP_FOUND: [],
            defs.BTP_EV_BAP_ASE_FOUND: [],
            defs.BTP_EV_BAP_STREAM_RECEIVED: [],
            defs.BTP_EV_BAP_BAA_FOUND: [],
            defs.BTP_EV_BAP_BIS_FOUND: [],
            defs.BTP_EV_BAP_BIS_SYNCED: [],
            defs.BTP_EV_BAP_BIS_STREAM_RECEIVED: [],
            defs.BTP_EV_BAP_SCAN_DELEGATOR_FOUND: [],
            defs.BTP_EV_BAP_BROADCAST_RECEIVE_STATE: [],
            defs.BTP_EV_BAP_PA_SYNC_REQ: [],
        }
        self.event_handlers = {
            defs.BTP_EV_BAP_DISCOVERY_COMPLETED: self._ev_discovery_completed,
        }
    ...
    def wait_codec_cap_found_ev(self, addr_type, addr, pac_dir, timeout, remove=False):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_BAP_CODEC_CAP_FOUND],
    ...
    def wait_discovery_completed_ev(self, addr_type, addr, timeout, remove=True):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_BAP_DISCOVERY_COMPLETED],
    ...
    def wait_ase_found_ev(self, addr_type, addr, ase_dir, timeout, remove=False):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_BAP_ASE_FOUND],
    ...
    def wait_stream_received_ev(self, addr_type, addr, ase_id, timeout, remove=True):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_BAP_STREAM_RECEIVED],
    ...
    def wait_bis_stream_received_ev(self, broadcast_id, bis_id, timeout, remove=True):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_BAP_BIS_STREAM_RECEIVED],
            lambda ev: (broadcast_id, bis_id) == (ev['broadcast_id'], ev['bis_id']),
            timeout, remove)

    def wait_scan_delegator_found_ev(self, addr_type, addr, timeout, remove=False):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_BAP_SCAN_DELEGATOR_FOUND],
            lambda ev: (addr_type, addr) == (ev["addr_type"], ev["addr"]),
            timeout, remove)
    ...
    def wait_broadcast_receive_state_ev(self, broadcast_id, peer_addr_type, peer_addr,
        ...
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_BAP_BROADCAST_RECEIVE_STATE],
    ...
    def wait_pa_sync_req_ev(self, addr_type, addr, timeout, remove=False):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_BAP_PA_SYNC_REQ],
            lambda ev: (addr_type, addr) == (ev["addr_type"], ev["addr"]),
            timeout, remove)
