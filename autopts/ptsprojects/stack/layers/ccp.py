...
class CCP:
    def __init__(self):
        self.events = {
            defs.BTP_EV_CCP_DISCOVERED:  { 'count': 0, 'status': 0, 'tbs_count': 0, 'gtbs': False },
            defs.BTP_EV_CCP_CALL_STATES: { 'count': 0, 'status': 0, 'index': 0, 'call_count': 0, 'states': [] },
            defs.BTP_EV_CCP_CHRC_HANDLES: [],
            defs.BTP_EV_CCP_CHRC_VAL: [],
            defs.BTP_EV_CCP_CHRC_STR: [],
            defs.BTP_EV_CCP_CP: [],
            defs.BTP_EV_CCP_CURRENT_CALLS: [],
        }
    ...
    def wait_characteristic_value_ev(self, addr_type, addr, timeout, remove=True):
        return wait_for_queue_event(
            self.events[defs.BTP_EV_CCP_CHRC_VAL],
    ...
    def wait_characteristic_str_ev(self, addr_type, addr, timeout, remove=True):
        return wait_for_queue_event(
            self.events[defs.BTP_EV_CCP_CHRC_STR],
            lambda _addr_type, _addr, *_:
            (addr_type, addr) == (_addr_type, _addr),
            timeout, remove)
    ...
    def wait_chrc_handles_ev(self, addr_type, addr, timeout, remove=True):
        return wait_for_queue_event(
            self.events[defs.BTP_EV_CCP_CHRC_HANDLES],
            lambda _addr_type, _addr, *_:
            (addr_type, addr) == (_addr_type, _addr),
            timeout, remove)
    ...
    def wait_cp_ev(self, addr_type, addr, timeout, remove=True):
        return wait_for_queue_event(
            self.events[defs.BTP_EV_CCP_CP],
            lambda _addr_type, _addr, *_:
            (addr_type, addr) == (_addr_type, _addr),
            timeout, remove)
    ...
    def wait_current_ev(self, addr_type, addr, timeout, remove=True):
        return wait_for_queue_event(
            self.events[defs.BTP_EV_CCP_CURRENT_CALLS],
            lambda _addr_type, _addr, *_:
            (addr_type, addr) == (_addr_type, _addr),
            timeout, remove)
