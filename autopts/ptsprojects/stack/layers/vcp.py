...
class VCP:
    def __init__(self):
        self.wid_counter = 0
        self.event_queues = {
            defs.BTP_EV_VCP_DISCOVERED: [],
            defs.BTP_EV_VCP_STATE: [],
            defs.BTP_EV_VCP_FLAGS: [],
            defs.BTP_EV_VCP_PROCEDURE: [],
        }
    ...
    def wait_discovery_completed_ev(self, addr_type, addr, timeout, remove=True):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_VCP_DISCOVERED],
    ...
    def wait_vcp_state_ev(self, addr_type, addr, timeout, remove=True):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_VCP_STATE],
            lambda _addr_type, _addr, *_:
            (addr_type, addr) == (_addr_type, _addr),
            timeout, remove)
    ...
    def wait_vcp_flags_ev(self, addr_type, addr, timeout, remove=True):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_VCP_FLAGS],
            lambda _addr_type, _addr, *_:
            (addr_type, addr) == (_addr_type, _addr),
            timeout, remove)
    ...
    def wait_vcp_procedure_ev(self, addr_type, addr, timeout, remove=True):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_VCP_PROCEDURE],
            lambda _addr_type, _addr, *_:
            (addr_type, addr) == (_addr_type, _addr),
            timeout, remove)
