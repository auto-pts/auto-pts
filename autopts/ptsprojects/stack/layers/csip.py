...
class CSIP:
    def __init__(self):
        ...
        self.wid_cnt = 0
        self.event_queues = {
            defs.BTP_EV_CSIP_DISCOVERED: [],
            defs.BTP_EV_CSIP_SIRK: [],
            defs.BTP_EV_CSIP_LOCK: []
        }
    ...
    def wait_discovery_completed_ev(self, addr_type, addr, timeout, remove=True):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_CSIP_DISCOVERED],
    ...
    def wait_sirk_ev(self, addr_type, addr, timeout, remove=True):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_CSIP_SIRK],
            lambda _addr_type, _addr, *_:
            (addr_type, addr) == (_addr_type, _addr),
            timeout, remove)
    ...
    def wait_lock_ev(self, timeout, remove=True):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_CSIP_LOCK],
