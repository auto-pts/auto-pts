...
class HAP:
    ...
    def __init__(self):
        self.peers = {}
        self.event_queues = {
            defs.BTP_EV_HAP_IAC_DISCOVERY_COMPLETE: [],
            defs.BTP_EV_HAP_HAUC_DISCOVERY_COMPLETE: [],
            defs.BTP_EV_HAP_PRESET_CHANGED: [],
        }
        self.event_handlers = {
            defs.BTP_EV_HAP_HAUC_DISCOVERY_COMPLETE: self._ev_hauc_discovery_complete,
        }
    ...
    def wait_iac_discovery_complete_ev(self, addr_type, addr, timeout, remove=True):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_HAP_IAC_DISCOVERY_COMPLETE],
    ...
    def wait_hauc_discovery_complete_ev(self, addr_type, addr, timeout, remove=True):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_HAP_HAUC_DISCOVERY_COMPLETE],
            lambda _addr_type, _addr, *_:
                (addr_type, addr) == (_addr_type, _addr),
            timeout, remove)
    ...
    def wait_preset_changed_ev(self, addr_type, addr, timeout, change_id, remove=True):
        return wait_for_queue_event(
            self.event_queues[defs.BTP_EV_HAP_PRESET_CHANGED],
