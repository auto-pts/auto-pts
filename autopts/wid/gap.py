...
def hdl_wid_103(params: WIDParams):
    ...
    if params.test_case_name in ['GAP/SEC/SEM/BV-09-C', 'GAP/SEC/SEM/BV-53-C']:
        ...
        if GAP_TEST_WID_103_ROUND == 0:
            ...
            if passkey != None:
                btp.gap_passkey_confirm_rsp(btp.pts_addr_get(), defs.BTP_BR_ADDRESS_TYPE, passkey)
            stack.l2cap_init(defs.BTP_BR_L2CAP_PSM_2, stack.l2cap.initial_mtu)
            btp.l2cap_conn(None, defs.BTP_BR_ADDRESS_TYPE, stack.l2cap.psm, stack.l2cap.initial_mtu)
        else:
            stack.l2cap_init(defs.BTP_BR_L2CAP_PSM, stack.l2cap.initial_mtu)
            btp.l2cap_conn(None, defs.BTP_BR_ADDRESS_TYPE, stack.l2cap.psm, stack.l2cap.initial_mtu)
    else:
...
def hdl_wid_273(params: WIDParams):
    ...
    if params.test_case_name in ['GAP/SEC/SEM/BI-25-C']:
        btp.gap_pair_v2(bd_addr_type=defs.BTP_BR_ADDRESS_TYPE, level=defs.BTP_GAP_CMD_PAIR_V2_MODE_2)
        return True

    if params.test_case_name in ['GAP/SEC/SEM/BI-26-C']:
        btp.gap_pair_v2(bd_addr_type=defs.BTP_BR_ADDRESS_TYPE, level=defs.BTP_GAP_CMD_PAIR_V2_MODE_3)
        return True

    btp.gap_pair_v2(bd_addr_type=defs.BTP_BR_ADDRESS_TYPE, level=defs.BTP_GAP_CMD_PAIR_V2_MODE_4)
    return True
