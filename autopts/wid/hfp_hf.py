...
def hdl_wid_1(params: WIDParams):
    ...
    if params.test_case_name in ["HFP/HF/CGSIT/SFC/BV-01-C", "IOPT/HFP/HF/CGSIT/SFC/BV-01-I"]:
...
def hdl_wid_8(_: WIDParams):
    ...
    return stack.hfp_hf.wait_for_call_state(bd_addr, call_index, defs.BTP_HFP_HF_CALL_STATUS_ACTIVE)
...
def hdl_wid_9(params: WIDParams):
    ...
    call_index = 0
    if not stack.hfp_hf.wait_for_call_count(bd_addr, 1):
        return False
...
def hdl_wid_10(_: WIDParams):
    ...
    call_count = stack.hfp_hf.get_call_count(bd_addr)
    if call_count < 1:
        return False
    ...
    return stack.hfp_hf.wait_for_call_count(bd_addr, call_count - 1)
...
def hdl_wid_21(_: WIDParams):
    ...
    return stack.hfp_hf.wait_for_call_count_gte(bd_addr, call_count + 1)
...
def hdl_wid_23(_: WIDParams):
    ...
    return stack.hfp_hf.wait_for_call_count(bd_addr, call_count)
...
def hdl_wid_24(params: WIDParams):
    ...
    if params.test_case_name in ['HFP/HF/OCL/BV-02-C']:
        return not stack.hfp_hf.wait_for_call_count_not_equal(bd_addr, call_count)
...
def hdl_wid_115(_: WIDParams):
    ...
    if call_count == 0:
        ...
        return stack.hfp_hf.wait_for_call_count(bd_addr, 1)
    else:
        call_index = 0
        btp.hfp_hf_release_specified_call(call_index)
        return stack.hfp_hf.wait_for_call_count(bd_addr, call_count - 1)
...
def hdl_wid_139(_: WIDParams):
    ...
    return stack.hfp_hf.wait_for_call_state(bd_addr, call_index,
                                            defs.BTP_HFP_HF_CALL_STATUS_INCOMING_HELD)
...
def hdl_wid_140(_: WIDParams):
    ...
    return stack.hfp_hf.wait_for_call_state(bd_addr, call_index,
                                            defs.BTP_HFP_HF_CALL_STATUS_ACTIVE)
