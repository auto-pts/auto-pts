...
def hdl_wid_32(_: WIDParams):
    ...
    Description: The Implementation Under Test(IUT) should disconnect the active L2CAP channel by sending a disconnect request to PTS after monitor timer expired.
    '''
    return True
...
def hdl_wid_24(_: WIDParams):
    '''
    Using the Implementation Under Test(IUT), queue up and send two(2) extended control field data(I - Frames) to the PTS.
    '''
    l2cap = get_stack().l2cap
    for i in range(0,2):
        for channel in l2cap.channels:
