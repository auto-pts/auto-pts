...
def hdl_wid_22(params: WIDParams):
    ...
    # `or` would be wrong here: BIPConnType.PRIM_IMAGE_PUSH is 0, so an
    # explicit mapping to it is falsy and would fall through to the default.
    # That happens to yield the same value today, but silently picks the
    # default the moment the default stops being PRIM_IMAGE_PUSH.
    conn_type = lookup(prim_conn_type)
    if conn_type is None:
        conn_type = BIPConnType.PRIM_IMAGE_PUSH
    ...
    if tc in ["BIP/CL/GOEP/SRMP/BV-01-C"]:
        ...
        return btp.bip_wait_for_operation_complete(
            event=defs.BTP_BIP_EV_CLIENT_PUT_IMAGE_RSP,
            rsp_code=OBEXRspCode.SUCCESS, timeout=60)
    return True
...
def hdl_wid_34(params: WIDParams):
    ...
    if not btp.bip_wait_for_operation_complete(
            event=defs.BTP_BIP_EV_CLIENT_PUT_LINKED_THUMBNAIL_RSP,
            rsp_code=OBEXRspCode.SUCCESS, timeout=60):
    ...
    if params.test_case_name == "BIP/IPSI/MFS/BV-05-C":
        ...
        if not btp.bip_wait_for_operation_complete(
                event=defs.BTP_BIP_EV_CLIENT_PUT_LINKED_ATTACHMENT_RSP,
                rsp_code=OBEXRspCode.SUCCESS, timeout=60):
...
def hdl_wid_35(params: WIDParams):
    ...
    if not btp.bip_wait_for_sdp_finished():
        logging.error('hdl_wid_35: SDP discovery for %r did not finish', uuid)
        return False

    sdp_conn = btp.bip_get_sdp_connection()
    if sdp_conn is None:
        logging.error('hdl_wid_35: no SDP record found for %r', uuid)
        return False
    ...
    if sdp_conn.l2cap_psm:
        ...
        # Neither transport advertised: the MMI asks for a channel that cannot be
        # opened, so say so instead of reporting the WID as handled.
        logging.error('hdl_wid_35: the SDP record for %r advertises neither a '
                      'GOEP L2CAP PSM nor an RFCOMM channel', uuid)
        return False

    if not btp.bip_wait_for_transport_connected():
        logging.error('hdl_wid_35: the secondary transport did not come up')
        return False

    if params.test_case_name in ["BIP/AAR/MFS/BV-07-C", "BIP/AAR/MFS/BV-10-C",
                                 "BIP/AAR/MFS/BV-11-C"]:
        btp.bip_second_connect(conn_type=conn_type)
        if not btp.bip_wait_for_client_connected(role=conn_role):
            logging.error('hdl_wid_35: the secondary OBEX connection was not '
                          'established')
            return False
...
def hdl_wid_38(params: WIDParams):
    ...
    return btp.bip_wait_for_operation_complete(
        event=defs.BTP_BIP_EV_CLIENT_GET_PARTIAL_IMAGE_RSP,
        rsp_code=OBEXRspCode.SUCCESS)
...
def hdl_wid_41(params: WIDParams):
    ...
    if params.test_case_name == "BIP/RDI/MFS/BV-25-C":
        # get_last_image_list() is None until a GetImagesList response has been
        # parsed, and the listing may legitimately be empty, so neither
        # subscripting nor len() can be taken for granted here. Report the MMI
        # as failed rather than raise: there is no image to select.
        handles = get_stack().bip.image_db.get_last_image_list()
        if not handles:
            logging.error('hdl_wid_41: no image list to select a handle from')
            return False
        handle = handles[0]
...
def hdl_wid_4004(params: WIDParams):
    ...
    """
    # Wait on the session we actually answered: accepting the secondary
    # (Referenced/Archived Objects) CONNECT marks the SECONDARY session
    # connected, so waiting on PRIMARY here would time out whenever the
    # primary OBEX connection is not (yet) up - and would silently report
    # success when an unrelated primary connection happened to be up.
