...
def hdl_wid_102(params: WIDParams):
    '''
    Please send an HCI connect request to establish a basic rate connection after the IUT discovers the Lower Tester over BR and LE.
    '''
...
def hdl_wid_6007(_: WIDParams):
    ...
    if SDP_RECORD_HANDLE != 0:
        btp.sdp_attr_req(service_record_handle=SDP_RECORD_HANDLE)
    else:
        return False
    return True
