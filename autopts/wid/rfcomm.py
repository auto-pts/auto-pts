...
def hdl_wid_12(params: WIDParams):
    ...
    try:
        # Send SABM command for RFCOMM control channel
        # Typically DLCI 0 is used for control channel
        btp.rfcomm_conn(bd_addr_type=defs.BTP_BR_ADDRESS_TYPE, channel=15)

        log("SABM operation for RFCOMM control channel initiated successfully")
        return True
    except Exception as e:
        log(f"Failed to initiate SABM operation: {e}")
        return False
