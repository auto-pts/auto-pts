...
from autopts.pybtp.types import A2DPCodecType, WIDParams
...
def hdl_wid_1020(params: WIDParams):
    ...
    if btp.a2dp_get_mmi_round(1020) == 0:
        ...
        remote_id = 1
        eps = btp.a2dp_get_endpoints()
        if eps:
            for ep in eps:
                cap = btp.a2dp_get_endpoints_media_type(endpoint_id=ep.ep_id)
                if cap:
                    if cap['codec_type'] == A2DPCodecType.SBC:
                        remote_id = ep.ep_id
                        break
        btp.a2dp_stream_config(remote_ep_id=remote_id)
...
def hdl_wid_1029(params: WIDParams):
    ...
    """
    btp.a2dp_stream_release()
    btp.a2dp_wait_for_stream_released(timeout=20)
...
def hdl_wid_1032(params: WIDParams):
    ...
    if params.test_case_name in ['A2DP/SNK/SYN/BV-01-C']:
        btp.a2dp_stream_delay_report(delay=1)
        btp.a2dp_wait_for_stream_state(expected_state='delay_reported')
        return True
