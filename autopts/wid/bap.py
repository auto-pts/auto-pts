...
class AudioConfigCis:
    ...
    Attributes:
        sink_channels (int): Number of channels for the sink direction
                             (0 indicates unidirectional CIS)
        source_channels (int): Number of channels for the source direction
                               (0 indicates unidirectional CIS)
    """
...
class AudioConfigTest:
    ...
    def connect_cis_in_qos(self) -> bool:
        # If both the sink and the source ASEs are connecting the in QoS configured state,
        # we set a shorthand check for that
        return self.connect_sink_in_qos and self.connect_source_in_qos

    @property
    def server_count(self) -> bool:
    ...
    def get_server(self, test_case: str) -> AudioConfigServer:
        ...
        return self.audio_config.value.servers[0 if "LT2" not in test_case else 1]


# Configuration used for Unicast Client tests, that indicate which audio configuration to use
# for each test, as well as which state we should connect the CIS for that direction it.
...
def create_ase_config(
    ...
    audio_config_test (AudioConfigTest): Reference to the Audio Configuration that is use with this
                                         config
    """
    ...
    # If connecting the CIS when sink ASEs are in the QoS state, and connect_source_in_qos is False,
    # then we need to move all sinks to the enabling state, or vice versa for source ASEs
...
def hdl_wid_311(params: WIDParams):
    ...
    ase_configs = []
    audio_config_test: AudioConfigTest = AC_CONFIGS_DICT[params.test_case_name.removesuffix("_LT2")]
