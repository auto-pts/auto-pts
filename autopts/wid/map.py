...
def hdl_wid_1(_: WIDParams):
    ...
    hdr = {
        ...
        OBEXHdr.APP_PARAM: {
            MAPAppParam.STATUS_IND: MAPMsgStatusInd.DELETED_STATUS,
            MAPAppParam.STATUS_VAL: 1,
        },
...
def hdl_wid_5(_: WIDParams):
    ...
    hdr = {
        ...
        OBEXHdr.APP_PARAM: {
            MAPAppParam.MAS_INST_ID: default_mce_mas(),
        }
    }
    encoded_hdr, _, _ = btp.map_enc_hdr(hdr, mopl - 3, 0)
    btp.map_set_info(MAPInfo.LOCAL_SRM, is_l2cap, default_mce_mas())
...
def hdl_wid_9(params: WIDParams):
    ...
    hdr = {
        ...
        OBEXHdr.NAME: "",
        OBEXHdr.APP_PARAM: {
            MAPAppParam.FILTER_MSG_TYPE: msg_type,
        }
    }
    encoded_hdr, _, _ = btp.map_enc_hdr(hdr, mopl - 3, 0)
    btp.map_set_info(MAPInfo.LOCAL_SRM, is_l2cap, default_mce_mas())
...
def hdl_wid_27(_: WIDParams):
    ...
    version = "1.1" if supported_features & MAPSupportedFeatures.MSG_FORMAT_V11 else "1.0"
...
def hdl_wid_28(params: WIDParams):
    ...
    if params.test_case_name.startswith("MAP/MCE/MFMH"):
        ...
        if not filtered:
            return False
        hdr[OBEXHdr.APP_PARAM][MAPAppParam.MSG_HANDLE] = filtered[0]['handle']
...
def hdl_wid_31(_: WIDParams):
    ...
    event = MAPEvent(
                event_type=MAPEventType.NEW_MSG,
                handle=msg.handle,
                folder=msg.folder,
    ...
    hdr = {
        ...
        OBEXHdr.APP_PARAM: {
            MAPAppParam.MAS_INST_ID: default_mse_mas(),
        },
...
def hdl_wid_46(_: WIDParams):
    ...
    hdr = {
        ...
        OBEXHdr.TYPE: MAPType.SET_MSG_STATUS,
        OBEXHdr.APP_PARAM: {
            MAPAppParam.STATUS_IND: MAPMsgStatusInd.READ_STATUS,
            MAPAppParam.STATUS_VAL: 1,  # Read
        },
        OBEXHdr.BODY: MAP_FILLER_BYTE,
    }
    encoded_hdr, _, _ = btp.map_enc_hdr(hdr, mopl - 3, 0)
...
def hdl_wid_64(_: WIDParams):
    ...
    hdr = {
        ...
        OBEXHdr.APP_PARAM: {
            MAPAppParam.CHARSET: 1,  # UTF-8
            MAPAppParam.CONVO_ID: convo_id,  # Valid conversation ID
        },
        OBEXHdr.BODY: msg_body,
    }
    encoded_hdr, remaining_hdr, offset = btp.map_enc_hdr(hdr, mopl - 3, 0)
...
def hdl_wid_66(_: WIDParams):
    ...
    hdr = {
        ...
        OBEXHdr.APP_PARAM: {
            MAPAppParam.PRESENCE_AVAIL: MAPPresenceAvailability.UNKNOWN,
...
def hdl_wid_67(_: WIDParams):
    ...
    event = MAPEvent(
                event_type=MAPEventType.CONVO_CHANGED,
                conversation_id=conversation_id,
                conversation_name=conv.name,
...
def hdl_wid_77(_: WIDParams):
    ...
    hdr = {
        ...
        OBEXHdr.APP_PARAM: {
            MAPAppParam.NTF_FILTER_MASK: 0x00007FFF,  # All events
        },
        OBEXHdr.BODY: MAP_FILLER_BYTE,
    }
    encoded_hdr, _, _ = btp.map_enc_hdr(hdr, mopl - 3, 0)
...
def hdl_wid_82(_: WIDParams):
    ...
    # Create event
    event = MAPEvent(
        event_type=MAPEventType.MSG_EXT_DATA_CHANGED,
        handle=msg.handle,
        folder=msg.folder,
        msg_type=msg.msg_type,
        datetime=msg.datetime,
...
def hdl_wid_83(_: WIDParams):
    ...
    event = MAPEvent(
        event_type=MAPEventType.MSG_REMOVED,
        handle=msg.handle,
        folder=msg.folder,
        msg_type=msg.msg_type,
...
def hdl_wid_87(_: WIDParams):
    ...
    hdr = {
        ...
        OBEXHdr.TYPE: MAPType.SET_MSG_STATUS,
        OBEXHdr.APP_PARAM: {
            MAPAppParam.STATUS_IND: MAPMsgStatusInd.SET_EXTENDED_DATA,
            MAPAppParam.EXT_DATA: "0:54;",  # 54 Facebook likes
        },
        OBEXHdr.BODY: MAP_FILLER_BYTE,
    }
    encoded_hdr, _, _ = btp.map_enc_hdr(hdr, mopl - 3, 0)
