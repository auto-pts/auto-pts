...
TMAP = {
    'read_supported_cmds': (defs.BTP_SERVICE_ID_TMAP,
                            defs.BTP_CMD_TMAP_READ_SUPPORTED_COMMANDS,
                            CONTROLLER_INDEX),
    'discover': (defs.BTP_SERVICE_ID_TMAP,
                      defs.BTP_CMD_TMAP_DISCOVER,
                      CONTROLLER_INDEX),
}
...
def tmap_ev_discovery_completed(tmap, data, data_len):
    ...
    tmap.event_received(defs.BTP_EV_TMAP_DISCOVERY_COMPLETED, (addr_type, addr, status, role))


TMAP_EV = {
    defs.BTP_EV_TMAP_DISCOVERY_COMPLETED: tmap_ev_discovery_completed,
}
