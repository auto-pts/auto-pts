...
CSIP = {
    'read_supported_cmds': (defs.BTP_SERVICE_ID_CSIP,
                            defs.BTP_CMD_CSIP_READ_SUPPORTED_COMMANDS,
                            CONTROLLER_INDEX),
    'discover': (defs.BTP_SERVICE_ID_CSIP, defs.BTP_CMD_CSIP_DISCOVER,
                 CONTROLLER_INDEX),
    'start_ordered_access': (defs.BTP_SERVICE_ID_CSIP,
                             defs.BTP_CMD_CSIP_START_ORDERED_ACCESS,
                             CONTROLLER_INDEX),
    'set_coordinator_lock': (defs.BTP_SERVICE_ID_CSIP,
                             defs.BTP_CMD_CSIP_SET_COORDINATOR_LOCK,
                             CONTROLLER_INDEX),
    'set_coordinator_release': (defs.BTP_SERVICE_ID_CSIP,
                                defs.BTP_CMD_CSIP_SET_COORDINATOR_RELEASE,
                                CONTROLLER_INDEX),
}
...
def csip_ev_discovery_completed(csip, data, data_len):
    ...
    csip.event_received(defs.BTP_EV_CSIP_DISCOVERED, (addr_type, addr, status,
...
def csip_sirk_ev(csip, data, data_len):
    ...
    csip.event_received(defs.BTP_EV_CSIP_SIRK, (addr_type, addr, sirk_hex))
...
def csip_lock_ev(csip, data, data_len):
    ...
    csip.event_received(defs.BTP_EV_CSIP_LOCK, lock_val)


CSIP_EV = {
    defs.BTP_EV_CSIP_DISCOVERED: csip_ev_discovery_completed,
    defs.BTP_EV_CSIP_SIRK: csip_sirk_ev,
    defs.BTP_EV_CSIP_LOCK: csip_lock_ev,
}
