...
def map_enc_app_param(app_param_dict):
    ...
    app_param_map = {
        MAPAppParam.MAX_LIST_CNT: (2, False),
        MAPAppParam.LIST_START_OFFSET: (2, False),
        MAPAppParam.FILTER_MSG_TYPE: (1, False),
        ...
        MAPAppParam.NEW_MSG: (1, False),
        MAPAppParam.NTF_STATUS: (1, False),
        MAPAppParam.MAS_INST_ID: (1, False),
        MAPAppParam.PARAM_MASK: (4, False),
        MAPAppParam.FOLDER_LISTING_SIZE: (2, False),
        MAPAppParam.LISTING_SIZE: (2, False),
        MAPAppParam.SUBJECT_LEN: (1, False),
        MAPAppParam.CHARSET: (1, False),
        MAPAppParam.FRACTION_REQUEST: (1, False),
        MAPAppParam.FRACTION_DELIVER: (1, False),
        MAPAppParam.STATUS_IND: (1, False),
        MAPAppParam.STATUS_VAL: (1, False),
        MAPAppParam.MSE_TIME: (0, True),             # Variable length string
        MAPAppParam.DB_ID: (0, True),   # Variable length string
        MAPAppParam.CONVO_LISTING_VER_CNTR: (0, True),   # Variable length string (16 bytes)
        MAPAppParam.PRESENCE_AVAIL: (1, False),
        ...
        MAPAppParam.CONVO_ID: (0, True),      # Variable length string (16 bytes)
        ...
        MAPAppParam.NTF_FILTER_MASK: (4, False),
        MAPAppParam.CONVO_PARAM_MASK: (4, False),
        MAPAppParam.OWNER_UCI: (0, True),            # Variable length string
        MAPAppParam.EXT_DATA: (0, True),        # Variable length string
        MAPAppParam.MAP_SUPPORTED_FEATURES: (4, False),
        MAPAppParam.MSG_HANDLE: (0, True),       # Variable length string
...
def map_mce_mas_connect(instance_id, bd_addr=None, bd_addr_type=defs.BTP_BR_ADDRESS_TYPE):
    ...
    send_supp_feat = 1 if sdp.supported_features & MAPSupportedFeatures.SUPPORTED_FEATURES_CONN_REQ else 0
...
def _map_mse_mas_set_ntf_reg_ev(_map, data, data_len):
    ...
    if req_hdr.get(OBEXHdr.APP_PARAM):
        notification_status = req_hdr[OBEXHdr.APP_PARAM].get(MAPAppParam.NTF_STATUS)
...
def _map_mse_mas_get_folder_listing_ev(_map, data, data_len):
    ...
    if p[MAPInfo.TX_CNT] == 0:
        ...
        max_list_count = app_param.get(MAPAppParam.MAX_LIST_CNT, 0xFFFF)
...
def _map_mse_mas_get_msg_listing_ev(_map, data, data_len):
    ...
    if p[MAPInfo.TX_CNT] == 0:
        ...
        subject_length = app_param.get(MAPAppParam.SUBJECT_LEN, 0xFF)
        parameter_mask = app_param.get(MAPAppParam.PARAM_MASK, 0xFFFFFFFF)

        # Filter parameters
        filter_message_type = app_param.get(MAPAppParam.FILTER_MSG_TYPE, 0)
        ...
        conversation_id = app_param.get(MAPAppParam.CONVO_ID, "")

        folder_name = p[MAPInfo.RX_DATA].get(OBEXHdr.NAME, '')
        version_1_1 = p[MAPInfo.SUPPORTED_FEATURES] & MAPSupportedFeatures.MSG_LISTING_FORMAT_V11
        ...
        hdr = {OBEXHdr.APP_PARAM: {}}
        hdr[OBEXHdr.APP_PARAM][MAPAppParam.NEW_MSG] = 1
        ...
        if max_list_count == 0:
            if p[MAPInfo.SUPPORTED_FEATURES] & MAPSupportedFeatures.FOLDER_VER_CNTR:
            ...
            if p[MAPInfo.SUPPORTED_FEATURES] & MAPSupportedFeatures.DB_ID:
                database_identifier = _map.storage.get_database_identifier()
                hdr[OBEXHdr.APP_PARAM][MAPAppParam.DB_ID] = database_identifier
...
def _map_mse_mas_get_msg_ev(_map, data, data_len):
    ...
    if p[MAPInfo.TX_CNT] == 0:
        ...
        version_1_1 = p[MAPInfo.SUPPORTED_FEATURES] & MAPSupportedFeatures.MSG_FORMAT_V11
        version = "1.1" if version_1_1 else "1.0"
...
def _map_mse_mas_push_msg_ev(_map, data, data_len):
    ...
    if first and not final:
        ...
        message_handle = app_param.get(MAPAppParam.MSG_HANDLE, "")
...
def _map_mse_mas_set_msg_status_ev(_map, data, data_len):
    ...
    msg_handle = p[MAPInfo.RX_DATA].get(OBEXHdr.NAME, '')
    status_indicator = p[MAPInfo.RX_DATA].get(OBEXHdr.APP_PARAM, {}).get(MAPAppParam.STATUS_IND, None)
    status_value = p[MAPInfo.RX_DATA].get(OBEXHdr.APP_PARAM, {}).get(MAPAppParam.STATUS_VAL, None)
...
def _map_mse_mas_get_mas_inst_info_ev(_map, data, data_len):
    ...
    if p[MAPInfo.TX_CNT] == 0:
        mas_inst_id = p[MAPInfo.RX_DATA].get(OBEXHdr.APP_PARAM, {}).get(MAPAppParam.MAS_INST_ID, None)
        connections = _map.get_all_connections(addr)
        connected_instances = [conn.instance_id for conn in connections
                            if conn.obex_type == defs.BTP_MAP_EV_MSE_MAS_CONNECT]
...
def _map_mse_mas_set_owner_status_ev(_map, data, data_len):
    ...
    conversation_id = app_params.get(MAPAppParam.CONVO_ID, "")
    presence_availability = app_params.get(MAPAppParam.PRESENCE_AVAIL, None)
...
def _map_mse_mas_get_owner_status_ev(_map, data, data_len):
    ...
    if p[MAPInfo.TX_CNT] == 0:
        conversation_id = p[MAPInfo.RX_DATA].get(OBEXHdr.APP_PARAM, {}).get(MAPAppParam.CONVO_ID, "")
        ...
        hdr = {OBEXHdr.APP_PARAM: {}}
        hdr[OBEXHdr.APP_PARAM][MAPAppParam.PRESENCE_AVAIL] = owner_status.presence_availability
...
def _map_mse_mas_get_convo_listing_ev(_map, data, data_len):
    ...
    if p[MAPInfo.TX_CNT] == 0:
        ...
        conversation_id = app_param.get(MAPAppParam.CONVO_ID, "")
        conv_parameter_mask = app_param.get(MAPAppParam.CONVO_PARAM_MASK, 0xFFFFFFFF)
        ...
        if p[MAPInfo.SUPPORTED_FEATURES] & MAPSupportedFeatures.DB_ID:
            ...
            hdr[OBEXHdr.APP_PARAM][MAPAppParam.DB_ID] = "0" * 32  # 128-bit hex string
            logging.debug("Added Database Identifier (backward compatibility): zeros")

        if max_list_count == 0:
            if p[MAPInfo.SUPPORTED_FEATURES] & MAPSupportedFeatures.CONVO_VER_CNTR:
                conversation_listing_version_counter = _map.storage.get_conversation_listing_version_counter()
                hdr[OBEXHdr.APP_PARAM][MAPAppParam.CONVO_LISTING_VER_CNTR] = conversation_listing_version_counter
...
def _map_mse_mas_set_ntf_filter_ev(_map, data, data_len):
    ...
    ntf_filter_mask = req_hdr.get(OBEXHdr.APP_PARAM, {}).get(MAPAppParam.NTF_FILTER_MASK, None)
...
def _map_mse_mns_connect_ev(_map, data, data_len):
    ...
    event_version = "1.0"
    if supported_features & MAPSupportedFeatures.EXT_EVENT_REPORT_V12:
        event_version = "1.2"
    elif supported_features & MAPSupportedFeatures.EXT_EVENT_REPORT_V11:
        event_version = "1.1"
