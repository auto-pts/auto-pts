...
from autopts.ptsprojects.stack.layers.bip import (
    BIPObexRole,
    BIPSrmFlag,
    ImageAttachment,
    ImageRecord,
...
# The BIP-specific OBEX header ids, spelled through OBEXHdr here because that is
# the canonical table on the pybtp side. stack/layers/bip.py spells the same two
# values as literals for the stack layer; they are asserted equal by the unit
# tests (test/test_bip.py).
...
# ---------------------------------------------------------------------------
# OBEX wire format, owned by BIP
#
# These used to live in pybtp/types.py as "generic" obex_* helpers, but BIP was
# their only caller - and BIP's header encoding deliberately differs from what a
# generic encoder would do (see bip_add_headers below on the NUL terminators).
# Keeping the codecs next to the only profile that uses them means a change here
# cannot reach another profile, and reading BIP's wire format does not require
# reading a helper written for everybody. The bodies are unchanged from the
# types.py versions; an equivalence check against the pre-move snapshot covered
# the build path over 431 input combinations.
#
# The top two bits of an OBEX header id give its encoding (OBEX 1.5 §2.1).
# ---------------------------------------------------------------------------
...
def bip_parse_tlv(data: bytes | bytearray, tag_sizes: dict | None = None) -> dict:
    """Parse TLV-encoded data into a {tag: value} dict.

    Args:
        data: Raw TLV bytes.
        tag_sizes: Optional {tag_id: size} for typed decoding (same convention
            as bip_build_tlv). If None or tag not found, value is raw bytes.
    """
...
def bip_parse_headers(buf: bytes | bytearray, app_param_parser=None) -> dict:
    ...
    while index < buf_len:
        ...
        if enc == OBEX_HEADER_ENCODING_1_BYTE:
            ...
            if header_id in (OBEXHdr.APP_PARAM, OBEXHdr.AUTH_CHALLENGE, OBEXHdr.AUTH_RSP):
                tlv = bip_parse_tlv(raw)
                if header_id == OBEXHdr.APP_PARAM and app_param_parser:
                    tlv = app_param_parser(tlv)
                headers[header_id] = tlv
            elif header_id in (OBEXHdr.BODY, OBEXHdr.END_BODY) and header_id in headers:
                # OBEX allows a packet to carry more than one Body header, and
                # the object they carry is their payloads concatenated in wire
                # order. Returning a dict means a repeat would otherwise
                # overwrite its predecessor and the first chunk would vanish
                # silently - the received object just ends up shorter than it
                # was sent. Every other header id keeps last-wins: BIP does not
                # repeat any of them, and changing that would alter how the
                # existing handlers read them.
                headers[header_id] = headers[header_id] + raw
            else:
...
def bt_bip_add_header_app_param(tlv_dict: dict):
    ...
    Thin wrapper over bip_build_tlv: the only BIP-specific part is the
    tag->length table, and that already lives in BIPAppParamTag.TAG_SIZES
    (BIP v1.2.1 Table 5.3, all ten tags). This function used to carry its own
    TAG_1BYTE / TAG_2BYTE / TAG_4BYTE / TAG_16BYTE lists, i.e. a second copy of
    the same table - which is exactly the kind of duplication that lets the two
    drift apart when a tag is added.

    The encodings agreed apart from one case: for a tag that is in neither
    table, the old code packed an int as a single byte while obex_build_tlv
    packs it in its minimal big-endian width. TAG_SIZES covers every tag the
    spec defines, so reaching that branch means the caller passed a tag BIP has
    no encoding rule for - a caller bug under either behaviour.
    ...
    return bip_build_tlv(tlv_dict, BIPAppParamTag.TAG_SIZES)
...
def bip_connect_rsp(bd_addr=None, bd_addr_type=defs.BTP_BR_ADDRESS_TYPE,
    ...
    if rsp_code == OBEXRspCode.SUCCESS:
        ...
        # Only after the BTP command succeeded: send_wait_rsp() raises BTPError
        # on a failed status, and marking the session connected beforehand left
        # the stack believing an OBEX connection exists that the IUT never
        # answered, so every later WID saw a bogus "connected" session.
...
def bip_second_connect_rsp(bd_addr=None, bd_addr_type=defs.BTP_BR_ADDRESS_TYPE,
    ...
    if rsp_code == OBEXRspCode.SUCCESS:
        # See bip_connect_rsp(): record the session only once the BTP command
...
def bip_wait_for_operation_complete(bd_addr=None, bd_addr_type=defs.BTP_BR_ADDRESS_TYPE,
                                   ...
                                   role=BIPObexRole.PRIMARY):
    stack = get_stack()
    # pts_addr_get() already falls back to the stored PTS address when
    # bd_addr is None, and normalises it with bd_addr_convert() otherwise.
    # Guarding the call with "if bd_addr is None" would let a caller-supplied
    # address reach get_bip_connection() unnormalised, which never matches a
    # stored connection and makes the wait return None instead of timing out.
    bd_addr = pts_addr_get(bd_addr)
    return stack.bip.wait_for_operation_complete(bd_addr, event, rsp_code=rsp_code, timeout=timeout, role=role)


def _bip_parse_xml(xml_str, what):
    """Parse a BIP XML document, returning None when it is not well-formed.

    A malformed or truncated document is an expected outcome, not a bug: the
    BI test cases send one on purpose, and it also happens whenever a
    multi-packet body failed to reassemble. Either way the caller should see the
    same failure it already reports for a missing body, instead of an
    ET.ParseError propagating out of a BTP helper and up through the WID
    handler as a traceback.
    """
    try:
        return ET.fromstring(xml_str)
    except ET.ParseError as e:
        logging.error('%s is not well-formed XML (%s): %r', what, e, xml_str)
        return None
...
def bip_get_caps_format(bd_addr=None, bd_addr_type=defs.BTP_BR_ADDRESS_TYPE):
    ...
    root = _bip_parse_xml(caps_xml, 'GetCapabilities body')
    if root is None:
        return None, None
...
def bip_get_image_list_format(bd_addr=None, bd_addr_type=defs.BTP_BR_ADDRESS_TYPE):
    ...
    root = _bip_parse_xml(list_xml, 'GetImagesList body')
    if root is None:
        return None
...
def bip_get_image_properties_format(bd_addr=None,
    ...
    root = _bip_parse_xml(props_xml, 'GetImageProperties body')
    if root is None:
        return None, None, None, None
...
def bip_register_put_image(handle, bd_addr=None,
    ...
    *handle* is the Img-Handle allocated by the server in the PutImage
    ...
    db = get_stack().bip.image_db

    # One atomic step: reading last_put_image, completing the record and
    # publishing it into ds.images must not interleave with an auto-response on
    # the event thread that is reading ds.images (GetImagesList) or replacing
    # last_put_image. db.lock is an RLock, so ds.add_image() - which is not
    # itself a locked BIPImageDatabase method - and any locked method called
    # from here are both fine under it.
    with db.lock:
...
def _bip_set_pending_put(sess, cmd_key, data, offset):
    """Record the continuation state of a multi-packet PUT on its session.

    The session object is what holds that state, so with no session there is
    nothing for _send_continuation() to pick the transfer up from: the non-final
    first packet would go out and the peer would wait for a continuation that
    never arrives. Refuse before anything is sent instead.

    A PUT is only ever issued on a connected session, so a missing one means the
    OBEX connection was never registered (see bip_connect_rsp) rather than
    something the test case can recover from.
    """
    if sess is None:
        raise BTPError(f'No OBEX session to continue the multi-packet {cmd_key} on')
...
def bip_put_image(img, encoding, pixel, bd_addr=None,
    ...
    chunk_size = _body_chunk_size(sess, 'put_image', len(data), len(image_data))
    ...
    if is_last:
        ...
        final = 0
        _bip_set_pending_put(sess, 'put_image', image_data, chunk_size)
...
def bip_put_linked_thumbnail(bd_addr=None,
    ...
    # Held under db.lock so the fallback cannot see a database the event thread
    # is midway through updating (bip_register_put_image publishes the image and
    # its handle in one step under the same lock).
    with db.lock:
        img = db.last_put_image
        if img is None:
    ...
    conn = stack.bip.get_bip_connection(bd_addr_resolved)
    sess = conn.get_session(BIPObexRole.PRIMARY) if conn else None
    chunk_size = _body_chunk_size(sess, 'put_linked_thumbnail', len(payload),
                                  len(thumbnail_data))
    ...
    if is_last:
        ...
        final = 0
        _bip_set_pending_put(sess, 'put_linked_thumbnail', thumbnail_data, chunk_size)
...
def bip_put_linked_attachment(bd_addr=None,
    ...
    # Under db.lock for the same reason as bip_put_linked_thumbnail: the
    # resolution and the attachment read below must see one consistent database.
    with db.lock:
        img = db.last_put_image
    ...
    conn = stack.bip.get_bip_connection(bd_addr_resolved)
    sess = conn.get_session(BIPObexRole.PRIMARY) if conn else None
    chunk_size = _body_chunk_size(sess, 'put_linked_attachment', len(payload),
                                  len(attachment_data))
    ...
    if is_last:
        ...
        payload.extend(_obex_hdr_byte_seq(OBEXHdr.BODY, chunk))
        final = 0
        _bip_set_pending_put(sess, 'put_linked_attachment', attachment_data, chunk_size)
...
def _bip_ev_server_connect_req(bip, data, data_len):
    ...
    headers = bip_parse_headers(body)
    target = headers.get(OBEXHdr.TARGET)
    conn = bip.get_bip_connection(addr)
    if conn:
        sess = conn.get_session(BIPObexRole.PRIMARY)
        # The peer's MOPL bounds every response this session sends, the same way
        # it bounds requests on the client side.
        sess.peer_mopl = mopl
        if target is not None:
...
def _bip_ev_server_data_req(ev_id, name, bip, data, data_len,
    ...
    bip.rx(addr, ev_id, (body,), role=role)
...
#
# Packet budget for multi-packet bodies.
#
# Everything a body has to share its OBEX packet with:
#
#   _OBEX_PKT_HDR_LEN  the opcode / response code (1) + packet length (2) that
#                      the stack pushes in front of whatever the tester is
#                      handed. bt_obex_put() pushes bt_obex_req_hdr and then
#                      obex_send() rejects buf->len > mopl with -EMSGSIZE
#                      (subsys/bluetooth/host/classic/obex.c), i.e. the MOPL
#                      covers these 3 bytes too. The old budget left them out
#                      entirely; harmless only because the budget itself was a
#                      hard-coded 150, far below any legal MOPL.
#   _BODY_HDR_LEN      the Body / End-of-Body header id (1) + length (2).
#   the operation prefix (Connection-Id / Type / SRM / SRMP) and whatever
#   operation-specific headers the caller already built - see
#   _op_prefix_len() and the len(data) terms at the call sites.
#
_OBEX_PKT_HDR_LEN = 3
_BODY_HDR_LEN = 3

# A BTP command also cannot exceed the tester's BTP MTU
# (tests/bluetooth/tester/src/btp/btp.h: BTP_MTU 1024, BTP_DATA_MAX_SIZE =
# BTP_MTU - sizeof(struct btp_hdr)); every command carrying a body spends 10 of
# those bytes on address (7) + final (1) + data_len (2). With today's
# OBEXSession.IUT_MAX_MOPL of 300 the transmit budget stays far below this, but
# the cap has to exist: a command over the BTP MTU is dropped by the tester
# ("Data len exceeds BTP MTU") rather than trimmed.
_BTP_MAX_OBEX_PAYLOAD = 1024 - 5 - 10


def _obex_payload_budget(sess):
    """Bytes of OBEX payload (headers + body) the next packet may carry.

    The transmit MOPL is owned by the OBEX session - see OBEXSession.tx_mopl(),
    which combines the peer's advertised MOPL with what the tester can actually
    build. Only the BTP MTU cap is applied on top of it here.
    """
    mopl = sess.tx_mopl() if sess is not None else OBEXSession.FALLBACK_MOPL

    return min(mopl - _OBEX_PKT_HDR_LEN, _BTP_MAX_OBEX_PAYLOAD)


def _op_prefix_len(sess, cmd_key):
    """Length of the prefix _bip_operation_cmd() will prepend to a first packet.

    Mirrors that function's prefix construction without mutating the SRM state,
    so a caller can budget for it before building the body. Keep the two in
    step: this is the one place the operation headers are counted rather than
    emitted.
    """
    length = 0

    if sess is not None and sess.conn_id is not None:
        length += 5  # header id + 4-byte Connection-Id

    type_val = BIP_CMD_TYPE_MAP.get(cmd_key)
    if type_val is not None:
        length += 3 + len(type_val)  # header id + 16-bit length + value

    if sess is not None and sess.is_srm_allowed() and \
       not (sess.srm_flags & BIPSrmFlag.SRM_LOCAL):
        length += 2  # SRM
        if sess.srmp_wait_count > 0:
            length += 2  # SRMP
...
def _body_chunk_size(sess, cmd_key, hdrs_len, body_len):
    """How much of *body_len* fits in the first packet of a PUT."""
    available = (_obex_payload_budget(sess) - _op_prefix_len(sess, cmd_key)
                 - hdrs_len - _BODY_HDR_LEN)

    return min(body_len, max(available, 1))


# Upper bound on the packets one multi-packet body may be split into, used to keep
# the two full-speed SRM continuation loops finite (the PUT one in
# _bip_ev_client_rsp() and the GET one in _send_get_rsp_with_srm()). At the
# smallest possible packet budget this still allows hundreds of KB, far beyond
# anything a BIP test case transfers.
_MAX_BODY_FRAGMENTS = 4096
...
def _send_continuation(addr, bd_addr_type=defs.BTP_BR_ADDRESS_TYPE,
                       role=BIPObexRole.PRIMARY):
    """Send the next packet of a multi-packet PUT.

    Returns True when a packet was sent, i.e. when the transfer advanced. The
    full-speed SRM loop in _bip_ev_client_rsp() relies on that return value to
    stop: the session it holds and the one looked up here are not guaranteed to
    be the same object, so it cannot tell from its own sess.pending_put whether
    this call made progress.

    *role* selects the OBEX session the pending transfer and the SRM state are
    read from. Only PRIMARY can reach this today (the secondary
    Referenced/Archived Objects connection carries GET and DELETE, never PUT),
    but the parameter is here so that the whole auto-response path takes the
    role from one place instead of some links in the chain hard-coding PRIMARY.
    """

    conn = get_stack().bip.get_bip_connection(addr)
    sess = conn.get_session(role) if conn else None
    ...
    # The prefix is decided first because it comes out of the same packet budget
    # as the body. A continuation packet repeats neither Connection-Id nor Type,
    # so SRMP is the only header that can precede the body here.
    prefix = bytearray()
    ...
    available = max(_obex_payload_budget(sess) - len(prefix) - _BODY_HDR_LEN, 1)
...
def _send_chunked_rsp(cmd_key, addr, chunk_key, body_data,
                      ...
                      srmp_wait=False, role=BIPObexRole.PRIMARY):
    """Send one packet of a chunked GET response on the *role* OBEX session.

    The chunk offsets and the SRMP budget live on that session, so a secondary
    (Referenced/Archived Objects) response must pass role=SECONDARY: the
    distinct chunk_key alone only keeps the offsets apart, not the SRM state.
    """
    ...
    # Build the headers that precede the body first: they come out of the same
    # packet budget. The budget used to be the flat _CHUNK_SIZE minus the Body
    # header only, so the first packet of a response could overrun it by the SRM
    # (2) + SRMP (2) + extra_headers bytes. Harmless at 150, not at a real MOPL.
    data = bytearray()
    ...
    available = max(_obex_payload_budget(sess) - len(data) - _BODY_HDR_LEN, 1)
    chunk_size = min(remaining, available)
...
def _update_srm_state(conn, srm, srmp, role=BIPObexRole.PRIMARY):
    """Update SRM state on the *role* OBEX session of a connection.

    Accepts the transport-level BIPConnection and operates on one of its OBEX
    sessions explicitly (no attribute proxies). Defaults to PRIMARY; the
    secondary server auto-responses pass SECONDARY so the two connections
    negotiate SRM independently.
...
def _send_get_rsp_with_srm(conn, cmd_key, addr, chunk_key, body_data,
                            ...
                            role=BIPObexRole.PRIMARY):
    # Accept the transport-level BIPConnection and operate on one of its OBEX
    # sessions explicitly (no attribute proxies). *role* defaults to PRIMARY;
    # the secondary server auto-responses pass SECONDARY so srm_flags and
    # srmp_wait_count are not shared with the primary imaging connection.
    #
    # The session is obtained with get_or_add_session() so that the rest of the
    # function can use it unconditionally. The previous "if conn else None"
    # produced a sess the body then dereferenced anyway - the very first branch
    # mixed a guarded "if sess: sess.reset_srm()" with an unguarded
    # "sess.chunk_offsets" one line above it - so the None case was never
    # actually handled. It is also not reachable: BIPConnection.__init__ creates
    # the PRIMARY session eagerly and nothing removes it, and a secondary
    # session exists from the moment its transport came up
    # (add_bip_connection(role=SECONDARY)).
    if conn is None:
        logging.error('_send_get_rsp_with_srm: no connection for %s, dropping '
                      'the %s response', addr, cmd_key)
        return

    sess = conn.get_or_add_session(role)
    logging.debug('_send_get_rsp_with_srm: role=%r sess=%r srm_allowed=%r '
                  'srm_flags=0x%02x srmp_wait_count=%r final=%r',
                  role, sess, sess.is_srm_allowed(), sess.srm_flags,
                  sess.srmp_wait_count, final)
    if not sess.is_srm_allowed() or not (sess.srm_flags & BIPSrmFlag.SRM_REMOTE):
        _send_chunked_rsp(cmd_key, addr, chunk_key, body_data,
                          extra_headers=extra_headers, role=role)
    ...
    if need_rsp:
        _send_chunked_rsp(cmd_key, addr, chunk_key, body_data,
                          ...
                          srmp_wait=srmp_wait, role=role)

    if sess.is_srm_full_speed() and final:
        # Same shape, and the same hazard, as the PUT continuation loop in
        # _bip_ev_client_rsp(): _send_chunked_rsp() re-resolves the session
        # through get_stack().bip, and if that yields a different object than
        # `sess` it advances offsets this loop never sees. Bound the iterations so
        # a mismatch fails the test case instead of wedging the event thread.
        for _ in range(_MAX_BODY_FRAGMENTS):
            if sess.chunk_offsets.get(chunk_key, 0) == 0:
                break
            _send_chunked_rsp(cmd_key, addr, chunk_key, body_data, role=role)
        else:
            logging.error('%s did not finish sending its body within %u '
                          'fragments, abandoning it', cmd_key,
                          _MAX_BODY_FRAGMENTS)
...
def _send_put_rsp_with_srm(conn, cmd_key, addr, final, rsp_code,
                            rsp_hdrs=b'', role=BIPObexRole.PRIMARY):
    # Accept the transport-level BIPConnection and operate on one of its OBEX
    # sessions explicitly (no attribute proxies). *role* defaults to PRIMARY;
    # no BIP PUT operation is defined on the secondary connection today, so
    # nothing passes SECONDARY yet, but the whole auto-response path now takes
    # the role from its caller instead of hard-coding it.
    sess = conn.get_session(role) if conn else None
    logging.debug('_send_put_rsp_with_srm: role=%r sess=%r srm_allowed=%r '
...
def _bip_ev_server_get_caps_req(bip, data, data_len,
    ...
    bip.rx(addr, ev_id, (final, headers), role=role)
    if bip.auto_response_enabled:
        ...
        if conn is None:
            ...
            return
        sess = conn.get_or_add_session(role)
        _update_srm_state(conn, srm, srmp, role=role)
        if sess.chunk_offsets.get(chunk_key, 0) == 0:
            sess._pending_body[chunk_key] = \
                bip.image_db.get_caps_rsp(final, headers)
    ...
    if bip.auto_response_enabled:
        ...
        if sess.chunk_offsets.get(chunk_key, 0) == 0:
            body_data, rsp_app_params, img_desc = \
                bip.image_db.get_image_list_rsp(final, headers)
        ...
        _send_get_rsp_with_srm(conn, rsp_cmd, addr,
                                ...
                                extra_headers=extra_hdrs, role=role)
...
def _bip_ev_server_get_image_properties_req(bip, data, data_len,
    ...
    if bip.auto_response_enabled:
        ...
        if sess.chunk_offsets.get(chunk_key, 0) == 0:
            sess._pending_body[chunk_key] = \
                bip.image_db.get_image_properties_rsp(final, headers)
        _send_get_rsp_with_srm(conn, rsp_cmd, addr,
                                chunk_key,
                                sess._pending_body[chunk_key], final,
                                role=role)
...
def _bip_ev_server_get_image_req(bip, data, data_len,
    ...
    if bip.auto_response_enabled:
        ...
        if sess.chunk_offsets.get(chunk_key, 0) == 0:
            sess._pending_body[chunk_key] = \
                bip.image_db.get_image_rsp(final, headers)
        ...
        _send_get_rsp_with_srm(conn, rsp_cmd, addr,
                                chunk_key, body_data, final,
                                extra_headers=bytes(length_hdr), role=role)
...
def _bip_ev_server_get_linked_thumbnail_req(bip, data, data_len,
    ...
    if bip.auto_response_enabled:
        ...
        if sess.chunk_offsets.get(chunk_key, 0) == 0:
            sess._pending_body[chunk_key] = \
                bip.image_db.get_linked_thumbnail_rsp(final, headers)
        _send_get_rsp_with_srm(conn, rsp_cmd, addr,
                                chunk_key,
                                sess._pending_body[chunk_key], final,
                                role=role)
...
def _bip_ev_server_get_linked_attachment_req(bip, data, data_len,
    ...
    if bip.auto_response_enabled:
        ...
        if sess.chunk_offsets.get(chunk_key, 0) == 0:
            sess._pending_body[chunk_key] = \
                bip.image_db.get_linked_attachment_rsp(final, headers)
        _send_get_rsp_with_srm(conn, rsp_cmd, addr,
                                chunk_key,
                                sess._pending_body[chunk_key], final,
                                role=role)
    ...
    if bip.auto_response_enabled:
        ...
        if sess.chunk_offsets.get(chunk_key, 0) == 0:
            body_data, rsp_app_params = \
                bip.image_db.get_partial_image_rsp(final, headers)
            extra_hdrs = bytearray()
    ...
    if bip.auto_response_enabled:
        ...
        if sess.chunk_offsets.get(chunk_key, 0) == 0:
            body_data, img_handle = \
                bip.image_db.get_monitoring_image_rsp(final, headers)
...
def _bip_ev_server_put_image_req(bip, data, data_len):
    ...
    if img_desc_raw:
        # Only decoding and inspecting the descriptor is guarded. The rejection
        # itself has to stay outside the try: _bip_operation_rsp_cmd() raises
        # BTPError when the BTP command fails, and swallowing that here used to
        # fall through to the auto-response below, which then sent a SECOND
        ...
        try:
            ...
            req_pixel = _parse_xml_attr(img_desc_xml, 'pixel')
            req_enc = _parse_xml_attr(img_desc_xml, 'encoding')
            reject = not bip.image_db.is_format_supported(req_enc, req_pixel)
...
# BTP_BIP_SECOND_*_RSP opcode routed to inst->second_server), a distinct
# chunk_key, and role=SECONDARY.
#
# The role is what keeps the two connections apart. The chunk_key only isolates
# the chunk offsets; srm_flags and srmp_wait_count live on the OBEX session, so
# without the role a secondary GET would negotiate SRM on the primary session
# and clobber the primary's state (and vice versa). The role also decides which
# session's rx queue the request event is enqueued on, which is what the
# SECONDARY waiters in wid/bip.py read.
...
def _bip_ev_client_connected(bip, data, data_len):
    ...
    if rsp_code == OBEXRspCode.SUCCESS:
        ...
        if conn is not None:
            ...
            # The negotiated MOPL is the fragment budget for every request sent
            # on this session; it used to be logged and dropped.
            sess.peer_mopl = mopl
...
def _bip_ev_client_rsp(ev_id, name, bip, data, data_len):
    ...
    if rsp_code == OBEXRspCode.CONTINUE:
        ...
        if ev_id == defs.BTP_BIP_EV_CLIENT_PUT_IMAGE_RSP:
            handle = bip_parse_headers(bytes(merged)).get(BIP_HDR_IMG_HANDLE)
            if handle:
    ...
    if get_cmd:
        ...
        if sess.is_srm_full_speed():
            # Under full-speed SRM the peer sends no CONTINUE per packet, so the
            # whole remainder of the transfer has to be pushed out here.
            #
            # Both stop conditions are needed. _send_continuation() returning
            # False is the real one: it looks the session up through
            # get_stack().bip, which is not necessarily the same object as the
            # `bip` this event was delivered on, so `sess.pending_put` may never
            # be cleared from under this loop no matter how many packets go out.
            # The iteration bound then covers whatever else could keep the
            # transfer from finishing - a stalled offset, a session swapped out
            # mid-transfer - by failing the test case instead of spinning
            # forever inside the single BIP event-handler thread, which would
            # wedge every later event for this run.
            for _ in range(_MAX_BODY_FRAGMENTS):
                if not _send_continuation(addr):
                    break
            else:
                logging.error('Multi-packet PUT did not finish within %u '
                              'fragments, abandoning the continuation',
                              _MAX_BODY_FRAGMENTS)
        else:
...
def bip_get_put_image_handle():
    db = get_stack().bip.image_db
    # Under db.lock: the handle is written by bip_register_put_image() on the
    # event thread as part of publishing the record, so reading the record and
    # its handle has to be one step to never return a half-registered handle.
    with db.lock:
        img = db.last_put_image
        return img.handle if img is not None else None
...
def _bip_ev_second_server_connect_req(bip, data, data_len):
    ...
    conn = bip.get_bip_connection(addr)
    if conn is not None:
        # Make sure the SECONDARY session exists before the event is enqueued
        # on it. It normally does already: the secondary transport-connected
        # event ran add_bip_connection(role=SECONDARY) before this OBEX CONNECT
        # could arrive. Creating it here keeps a lost/reordered transport event
        # from turning into a dropped CONNECT request that WID 4004 would then
        # never answer.
        sess = conn.get_or_add_session(BIPObexRole.SECONDARY)
        sess.peer_mopl = mopl
        if target is not None:
    ...
    bip.rx(addr, defs.BTP_BIP_EV_SECOND_SERVER_CONNECT_REQ,
           (version, mopl, body), role=BIPObexRole.SECONDARY)
...
def _bip_ev_second_server_disconnect_req(bip, data, data_len):
    _bip_ev_server_data_req(defs.BTP_BIP_EV_SECOND_SERVER_DISCONNECT_REQ,
                            ...
                            data_len, role=BIPObexRole.SECONDARY)
...
def _bip_ev_second_client_disconnected(bip, data, data_len):
    ...
    # remove_bip_obex_connection() only clears the OBEX state, the session
    # object itself survives, so the event still has a queue to land on.
    bip.rx(addr, defs.BTP_BIP_EV_SECOND_CLIENT_DISCONNECTED, (rsp_code, body),
           role=BIPObexRole.SECONDARY)


def _bip_ev_second_client_aborted(bip, data, data_len):
    _bip_ev_second_client_rsp(defs.BTP_BIP_EV_SECOND_CLIENT_ABORTED,
