...
GATTS = {
    'read_supported_cmds': (defs.BTP_SERVICE_ID_GATTS,
                            ...
                            CONTROLLER_INDEX),
    'register_database': (defs.BTP_SERVICE_ID_GATTS, defs.BTP_GATTS_CMD_REGISTER_DATABASE,
                          CONTROLLER_INDEX),
...
def gatt_sr_set_val(bd_addr_type, bd_addr, hdl, values):
    ...
    for i, handle in enumerate(hdl):
        ...
        # TODO: following line triggers issue (len-val returns 2 instead of 1)
...
def gatt_sr_register_database(db_id):
    logging.debug("%s %r", gatt_sr_register_database.__name__, db_id)
    ...
    iutctl.btp_socket.send(*GATTS['register_database'], data=data_ba)
