...
from autopts.pybtp.types import AVDTPServiceCategory, addr_str_to_le_bytes
...
def a2dp_get_all_capabilities_ev(a2dp, data, data_len):
    logging.debug("%s %r %r", a2dp_get_all_capabilities_ev.__name__, data, data_len)
...
def a2dp_wait_for_delay_reported(addr=None, stream_id=0, expected_delay=1, timeout=5):
    ...
    bd_addr_ba = addr_str_to_le_bytes(pts_addr_get(addr))
...
def a2dp_wait_for_discovered(addr=None, timeout=10):
...
A2DP_EV = {
    ...
    defs.BTP_A2DP_EV_GET_ALL_CAPABILITIES: a2dp_get_all_capabilities_ev,
