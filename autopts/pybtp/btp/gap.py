...
def gap_big_sync_lost_ev_(gap, data, data_len):
    ...
    stack.gap.big_sync_established_rxed = False
...
def gap_bis_stream_received_ev_(gap, data, data_len):
    ...
    if bis_id in stack.gap.big_bis_stream_rx.keys():
        streams = stack.gap.big_bis_stream_rx[bis_id]
    else:
        streams = []
    streams.append({'flags': flags, 'timestamp': ts, 'seq_num': seq_num, 'data': stream_data})
    stack.gap.big_bis_stream_rx[bis_id] = streams
