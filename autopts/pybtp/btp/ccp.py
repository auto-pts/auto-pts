...
def ccp_await_discovered(timeout=6000):
    ...
    stack = get_stack()
    success = ccp_await_event(stack.ccp, defs.BTP_EV_CCP_DISCOVERED, timeout)

    return success, \
           stack.ccp.events[defs.BTP_EV_CCP_DISCOVERED]['status'], \
           stack.ccp.events[defs.BTP_EV_CCP_DISCOVERED]['tbs_count'], \
           stack.ccp.events[defs.BTP_EV_CCP_DISCOVERED]['gtbs']
...
def ccp_ev_discovered(ccp, data, data_len):
    ...
    ccp.event_received(defs.BTP_EV_CCP_DISCOVERED, event_dict)
...
def ccp_ev_chrc_handles(ccp, data, data_len):
    ...
    ccp.event_received_2(defs.BTP_EV_CCP_CHRC_HANDLES, (provider_name, bearer_uci, bearer_technology,
...
def ccp_ev_chrc_val(ccp, data, data_len):
    ...
    ccp.event_received_2(defs.BTP_EV_CCP_CHRC_VAL, (addr_type, addr, status, inst_index,
                                                value))
...
def ccp_ev_chrc_str(ccp, data, data_len):
    ...
    ccp.event_received_2(defs.BTP_EV_CCP_CHRC_STR, (addr_type, addr, status, inst_index, str_data))
...
def ccp_ev_cp(ccp, data, data_len):
    ...
    ccp.event_received_2(defs.BTP_EV_CCP_CP, (addr_type, addr, status))
...
def ccp_ev_current_calls(ccp, data, data_len):
    ...
    ccp.event_received_2(defs.BTP_EV_CCP_CURRENT_CALLS, (addr_type, addr, status))
...
def ccp_await_call_state(timeout=1000):
    ...
    stack = get_stack()
    success = ccp_await_event(stack.ccp, defs.BTP_EV_CCP_CALL_STATES, timeout)

    return success, \
           stack.ccp.events[defs.BTP_EV_CCP_CALL_STATES]['status'], \
           stack.ccp.events[defs.BTP_EV_CCP_CALL_STATES]['index'], \
           stack.ccp.events[defs.BTP_EV_CCP_CALL_STATES]['call_count'], \
           stack.ccp.events[defs.BTP_EV_CCP_CALL_STATES]['states']
...
def ccp_ev_call_states(ccp, data, data_len):
    ...
    ccp.event_received(defs.BTP_EV_CCP_CALL_STATES, event_dict)


CCP_EV = {
    defs.BTP_EV_CCP_DISCOVERED: ccp_ev_discovered,
    defs.BTP_EV_CCP_CALL_STATES: ccp_ev_call_states,
    defs.BTP_EV_CCP_CHRC_HANDLES: ccp_ev_chrc_handles,
    defs.BTP_EV_CCP_CHRC_VAL: ccp_ev_chrc_val,
    defs.BTP_EV_CCP_CHRC_STR: ccp_ev_chrc_str,
    defs.BTP_EV_CCP_CP: ccp_ev_cp,
    defs.BTP_EV_CCP_CURRENT_CALLS: ccp_ev_current_calls,
}
