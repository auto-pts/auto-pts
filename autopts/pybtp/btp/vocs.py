...
VOCS = {
    'read_supported_cmds': (defs.BTP_SERVICE_ID_VCP,
                            defs.BTP_CMD_VOCS_READ_SUPPORTED_COMMANDS,
                            CONTROLLER_INDEX, ""),
    'audio_desc':          (defs.BTP_SERVICE_ID_VOCS, defs.BTP_CMD_VOCS_UPDATE_OUT_DESC,
                            CONTROLLER_INDEX),
    "audio_loc":           (defs.BTP_SERVICE_ID_VOCS, defs.BTP_CMD_VOCS_UPDATE_AUDIO_LOC,
                            CONTROLLER_INDEX),
    'state_read':          (defs.BTP_SERVICE_ID_VOCS,
                            defs.BTP_CMD_VOCS_OFFSET_STATE_GET,
                            CONTROLLER_INDEX),
    'audio_loc_get':       (defs.BTP_SERVICE_ID_VOCS,
                            defs.BTP_CMD_VOCS_AUDIO_LOC_GET,
                            CONTROLLER_INDEX),
    'state_set':           (defs.BTP_SERVICE_ID_VOCS,
                            defs.BTP_CMD_VOCS_OFFSET_STATE_SET,
                            CONTROLLER_INDEX),
}
...
def vocs_state_ev(vocs, data, data_len):
    ...
    vocs.event_received(defs.BTP_EV_VOCS_OFFSET, (addr_type, addr, att_status, offset))
...
def vocs_audio_loc_ev(vocs, data, data_len):
    ...
    vocs.event_received(defs.BTP_EV_VOCS_AUDIO_LOC, (addr_type, addr, att_status,
                                                 location))
...
def vocs_procedure_ev(vocs, data, data_len):
    ...
    vocs.event_received(defs.BTP_EV_VOCS_PROCEDURE, (addr_type, addr, att_status, opcode))


VOCS_EV = {
    defs.BTP_EV_VOCS_OFFSET: vocs_state_ev,
    defs.BTP_EV_VOCS_AUDIO_LOC: vocs_audio_loc_ev,
    defs.BTP_EV_VOCS_PROCEDURE: vocs_procedure_ev
}