...
L2CAP = {
    "read_supp_cmds": (defs.BTP_SERVICE_ID_L2CAP,
                       defs.BTP_CMD_L2CAP_READ_SUPPORTED_COMMANDS,
                       defs.BTP_INDEX_NONE, ""),
    "connect": (defs.BTP_SERVICE_ID_L2CAP, defs.BTP_CMD_L2CAP_CONNECT,
                CONTROLLER_INDEX),
    "disconnect": (defs.BTP_SERVICE_ID_L2CAP, defs.BTP_CMD_L2CAP_DISCONNECT,
                   CONTROLLER_INDEX),
    "send_data": (defs.BTP_SERVICE_ID_L2CAP, defs.BTP_CMD_L2CAP_SEND_DATA,
                  CONTROLLER_INDEX),
    "listen": (defs.BTP_SERVICE_ID_L2CAP, defs.BTP_CMD_L2CAP_LISTEN,
               CONTROLLER_INDEX),
    "reconfigure": (defs.BTP_SERVICE_ID_L2CAP, defs.BTP_CMD_L2CAP_RECONFIGURE,
                    CONTROLLER_INDEX),
    "disconnect_eatt_chans": (defs.BTP_SERVICE_ID_L2CAP, defs.BTP_CMD_L2CAP_DISCONNECT_EATT_CHANS,
                              CONTROLLER_INDEX),
    "credits": (defs.BTP_SERVICE_ID_L2CAP, defs.BTP_CMD_L2CAP_CREDITS,
                CONTROLLER_INDEX),
}
...
def l2cap_conn_rsp():
    ...
    btp_hdr_check(tuple_hdr, defs.BTP_SERVICE_ID_L2CAP, defs.BTP_CMD_L2CAP_CONNECT)
...
def l2cap_disconn(chan_id):
    ...
    l2cap_command_rsp_succ(defs.BTP_CMD_L2CAP_DISCONNECT)
...
def l2cap_listen(psm, transport, mtu=0, response=L2CAPConnectionResponse.success):
    ...
    l2cap_command_rsp_succ(defs.BTP_CMD_L2CAP_LISTEN)
...
def l2cap_disconn_eatt_chans(bd_addr, bd_addr_type, channel_count):
    ...
    l2cap_command_rsp_succ(defs.BTP_CMD_L2CAP_DISCONNECT_EATT_CHANS)
...
def l2cap_reconfigure(bd_addr, bd_addr_type, mtu, channels):
    ...
    l2cap_command_rsp_succ(defs.BTP_CMD_L2CAP_RECONFIGURE)
...
def l2cap_credits(chan_id):
    ...
    l2cap_command_rsp_succ(defs.BTP_CMD_L2CAP_CREDITS)
...
L2CAP_EV = {
    defs.BTP_EV_L2CAP_CONNECTED: l2cap_connected_ev,
    defs.BTP_EV_L2CAP_DISCONNECTED: l2cap_disconnected_ev,
    defs.BTP_EV_L2CAP_DATA_RECEIVED: l2cap_data_rcv_ev,
    defs.BTP_EV_L2CAP_RECONFIGURED: l2cap_reconfigured_ev,
}
