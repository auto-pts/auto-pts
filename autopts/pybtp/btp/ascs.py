...
ASCS = {
    'read_supported_cmds': (defs.BTP_SERVICE_ID_ASCS,
                            defs.BTP_CMD_ASCS_READ_SUPPORTED_COMMANDS,
                            CONTROLLER_INDEX),
    'config_codec': (defs.BTP_SERVICE_ID_ASCS, defs.BTP_CMD_ASCS_CONFIGURE_CODEC,
                     CONTROLLER_INDEX),
    'config_qos': (defs.BTP_SERVICE_ID_ASCS, defs.BTP_CMD_ASCS_CONFIGURE_QOS,
                   CONTROLLER_INDEX),
    'enable': (defs.BTP_SERVICE_ID_ASCS, defs.BTP_CMD_ASCS_ENABLE,
               CONTROLLER_INDEX),
    'start_ready': (defs.BTP_SERVICE_ID_ASCS, defs.BTP_CMD_ASCS_RECEIVER_START_READY,
                    CONTROLLER_INDEX),
    'stop_ready': (defs.BTP_SERVICE_ID_ASCS, defs.BTP_CMD_ASCS_RECEIVER_STOP_READY,
                   CONTROLLER_INDEX),
    'disable': (defs.BTP_SERVICE_ID_ASCS, defs.BTP_CMD_ASCS_DISABLE,
                CONTROLLER_INDEX),
    'release': (defs.BTP_SERVICE_ID_ASCS, defs.BTP_CMD_ASCS_RELEASE,
                CONTROLLER_INDEX),
    'update_metadata': (defs.BTP_SERVICE_ID_ASCS, defs.BTP_CMD_ASCS_UPDATE_METADATA,
                        CONTROLLER_INDEX),
    'add_ase_to_cis': (defs.BTP_SERVICE_ID_ASCS, defs.BTP_CMD_ASCS_ADD_ASE_TO_CIS,
                       CONTROLLER_INDEX),
    'preconfig_qos': (defs.BTP_SERVICE_ID_ASCS, defs.BTP_CMD_ASCS_PRECONFIG_QOS,
                      CONTROLLER_INDEX),
}
...
def ascs_ev_operation_completed_(ascs, data, data_len):
    ...
    ascs.event_received(defs.BTP_EV_ASCS_OPERATION_COMPLETED,
                        (addr_type, addr, ase_id, opcode, status, flags))
...
def ascs_ev_characteristic_subscribed_(ascs, data, data_len):
    ...
    ascs.event_received(defs.BTP_EV_ASCS_CHARACTERISTIC_SUBSCRIBED,
...
def ascs_ev_ase_state_changed_(ascs, data, data_len):
    ...
    ascs.event_received(defs.BTP_EV_ASCS_ASE_STATE_CHANGED,
                        (addr_type, addr, ase_id, state))


ASCS_EV = {
    defs.BTP_EV_ASCS_OPERATION_COMPLETED: ascs_ev_operation_completed_,
    defs.BTP_EV_ASCS_CHARACTERISTIC_SUBSCRIBED: ascs_ev_characteristic_subscribed_,
    defs.BTP_EV_ASCS_ASE_STATE_CHANGED: ascs_ev_ase_state_changed_,
}
