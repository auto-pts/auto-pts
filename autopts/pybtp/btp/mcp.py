...
def mcp_ev_discovery_completed(mcp, data, data_len):
    ...
    mcp.event_received(defs.BTP_EV_MCP_DISCOVERED, (addr_type, addr, status, player_name,
...
def mcp_track_duration_ev(mcp, data, data_len):
    ...
    mcp.event_received(defs.BTP_EV_MCP_TRACK_DURATION, (addr_type, addr, status,
                                                    duration))
...
def mcp_track_position_ev(mcp, data, data_len):
    ...
    mcp.event_received(defs.BTP_EV_MCP_TRACK_POSITION, (addr_type, addr, status,
                                                    position))
...
def mcp_playback_speed_ev(mcp, data, data_len):
    ...
    mcp.event_received(defs.BTP_EV_MCP_PLAYBACK_SPEED, (addr_type, addr, status,
                                                    speed))
...
def mcp_seeking_speed_ev(mcp, data, data_len):
    ...
    mcp.event_received(defs.BTP_EV_MCP_SEEKING_SPEED, (addr_type, addr, status,
                                                   speed))
...
def mcp_icon_obj_id_ev(mcp, data, data_len):
    ...
    mcp.event_received(defs.BTP_EV_MCP_ICON_OBJ_ID, (addr_type, addr, status,
                                                 obj_id))
...
def mcp_next_track_obj_id_ev(mcp, data, data_len):
    ...
    mcp.event_received(defs.BTP_EV_MCP_NEXT_TRACK_OBJ_ID, (addr_type, addr, status,
                                                       obj_id))
...
def mcp_parent_group_obj_id_ev(mcp, data, data_len):
    ...
    mcp.event_received(defs.BTP_EV_MCP_PARENT_GROUP_OBJ_ID, (addr_type, addr, status,
                                                         obj_id))
...
def mcp_current_group_obj_id_ev(mcp, data, data_len):
    ...
    mcp.event_received(defs.BTP_EV_MCP_CURRENT_GROUP_OBJ_ID, (addr_type, addr, status,
                                                          obj_id))
...
def mcp_playing_order_ev(mcp, data, data_len):
    ...
    mcp.event_received(defs.BTP_EV_MCP_PLAYING_ORDER, (addr_type, addr, status,
                                                   order))
...
def mcp_playing_orders_supported_ev(mcp, data, data_len):
    ...
    mcp.event_received(defs.BTP_EV_MCP_PLAYING_ORDERS_SUPPORTED, (addr_type, addr,
                                                              status, orders))
...
def mcp_media_state_ev(mcp, data, data_len):
    ...
    mcp.event_received(defs.BTP_EV_MCP_MEDIA_STATE, (addr_type, addr, status,
                                                 state))
...
def mcp_opcodes_supported_ev(mcp, data, data_len):
    ...
    mcp.event_received(defs.BTP_EV_MCP_OPCODES_SUPPORTED, (addr_type, addr,
                                                       status, opcodes))
...
def mcp_content_control_id_ev(mcp, data, data_len):
    ...
    mcp.event_received(defs.BTP_EV_MCP_CONTENT_CONTROL_ID, (addr_type, addr, status,
                                                        ccid))
...
def mcp_segments_obj_id_ev(mcp, data, data_len):
    ...
    mcp.event_received(defs.BTP_EV_MCP_SEGMENTS_OBJ_ID, (addr_type, addr, status,
                                                     obj_id))
...
def mcp_current_track_obj_id_ev(mcp, data, data_len):
    ...
    mcp.event_received(defs.BTP_EV_MCP_CURRENT_TRACK_OBJ_ID, (addr_type, addr, status,
                                                          obj_id))
...
def mcp_control_point_ev(mcp, data, data_len):
    ...
    mcp.event_received(defs.BTP_EV_MCP_COMMAND, (addr_type, addr, status, opcode,
                                             use_param, param))
...
def mcp_search_control_point_ev(mcp, data, data_len):
    ...
    mcp.event_received(defs.BTP_EV_MCP_SEARCH, (addr_type, addr, status, param_len,
                                            search, param))
...
def mcp_cmd_ntf_ev(mcp, data, data_len):
    ...
    mcp.event_received(defs.BTP_EV_MCP_CMD_NTF, (addr_type, addr, status,
...
def mcp_search_ntf_ev(mcp, data, data_len):
    ...
    mcp.event_received(defs.BTP_EV_MCP_SEARCH_NTF, (addr_type, addr, status,
                                                result_code))


MCP_EV = {
    defs.BTP_EV_MCP_DISCOVERED: mcp_ev_discovery_completed,
    defs.BTP_EV_MCP_TRACK_DURATION: mcp_track_duration_ev,
    defs.BTP_EV_MCP_TRACK_POSITION: mcp_track_position_ev,
    defs.BTP_EV_MCP_PLAYBACK_SPEED: mcp_playback_speed_ev,
    defs.BTP_EV_MCP_SEEKING_SPEED: mcp_seeking_speed_ev,
    defs.BTP_EV_MCP_ICON_OBJ_ID: mcp_icon_obj_id_ev,
    defs.BTP_EV_MCP_NEXT_TRACK_OBJ_ID: mcp_next_track_obj_id_ev,
    defs.BTP_EV_MCP_PARENT_GROUP_OBJ_ID: mcp_parent_group_obj_id_ev,
    defs.BTP_EV_MCP_CURRENT_GROUP_OBJ_ID: mcp_current_group_obj_id_ev,
    defs.BTP_EV_MCP_PLAYING_ORDER: mcp_playing_order_ev,
    defs.BTP_EV_MCP_PLAYING_ORDERS_SUPPORTED: mcp_playing_orders_supported_ev,
    defs.BTP_EV_MCP_MEDIA_STATE: mcp_media_state_ev,
    defs.BTP_EV_MCP_OPCODES_SUPPORTED: mcp_opcodes_supported_ev,
    defs.BTP_EV_MCP_CONTENT_CONTROL_ID: mcp_content_control_id_ev,
    defs.BTP_EV_MCP_SEGMENTS_OBJ_ID: mcp_segments_obj_id_ev,
    defs.BTP_EV_MCP_CURRENT_TRACK_OBJ_ID: mcp_current_track_obj_id_ev,
    defs.BTP_EV_MCP_COMMAND: mcp_control_point_ev,
    defs.BTP_EV_MCP_SEARCH: mcp_search_control_point_ev,
    defs.BTP_EV_MCP_CMD_NTF: mcp_cmd_ntf_ev,
    defs.BTP_EV_MCP_SEARCH_NTF: mcp_search_ntf_ev
}