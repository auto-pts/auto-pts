...
MICP = {
    'read_supported_cmds': (defs.BTP_SERVICE_ID_MICP,
                            defs.BTP_CMD_MICP_READ_SUPPORTED_COMMANDS,
                            CONTROLLER_INDEX, ""),
    'discovery':           (defs.BTP_SERVICE_ID_MICP,
                            defs.BTP_CMD_MICP_DISCOVERY,
                            CONTROLLER_INDEX),
    'mute_state':          (defs.BTP_SERVICE_ID_MICP,
                            defs.BTP_CMD_MICP_MUTE_STATE,
                            CONTROLLER_INDEX),
    'mute':                (defs.BTP_SERVICE_ID_MICP,
                            defs.BTP_CMD_MICP_MUTE,
                            CONTROLLER_INDEX),
}
...
def micp_ev_discovery_completed_(micp, data, data_len):
    ...
    micp.event_received(defs.BTP_EV_MICP_DISCOVERED, (addr_type, addr, att_status, mute_handle,
...
def micp_mute_state_ev(micp, data, data_len):
    ...
    micp.event_received(defs.BTP_EV_MICP_MUTE_STATE, (addr_type, addr, att_status, mute))


MICP_EV = {
    defs.BTP_EV_MICP_DISCOVERED: micp_ev_discovery_completed_,
    defs.BTP_EV_MICP_MUTE_STATE: micp_mute_state_ev,
}